    ...
    public int insertTAdvanceDelivery(TAdvanceDeliveryDTO tAdvanceDeliveryDTO) {
        ...
        if (!CollectionUtils.isEmpty(deliveryDetailList)) {
            deliveryDetailList.forEach(e -> {
                ...
                e.setDetectionCount(0L);
                e.setPutawayCount(0L);
            });
            ...
            deliveryDetailList.forEach(detail -> {
                // 生成用于构建RFID的批次号 单据批次号存在重复 不可使用
                String code = codeConfigService.getBatchCode1(detail.getMaterialId(), DateUtils.getNowDate());
                detail.setRfidBatchCode(code);

                //生成物料详情
                for (int i = 0; i < detail.getPredictCount(); i++) {
...
//        }
    ...
    public AjaxResult approveTAdvanceDelivery(TAdvanceDelivery tAdvanceDelivery) {
        ...
        if (Constants.INOUT_STATUS_FAILED.equals(tAdvanceDelivery.getStatus())) {
            //审核不通过，删除物料详情
            Long[] ids = new Long[1];
            ids[0] = tAdvanceDelivery.getId();
            materialDetailService.deleteTMaterialDetailByDeliveryIds(ids);
    ...
    public AjaxResult registerTAdvanceDelivery(TAdvanceDeliveryDTO tAdvanceDelivery) {
        ...
        //过滤掉小于等于0的数量
        deliveryDetailList = deliveryDetailList.stream().filter(e -> e.getRegistrationCount() != null && e.getRegistrationCount() > 0).collect(Collectors.toList());
        ...
        if (!CollectionUtils.isEmpty(deliveryDetailList)) {
            ...
            for (TAdvanceDeliveryDetail tAdvanceDeliveryDetail : deliveryDetailList) {
                ...
                Long oldReceiveCount = oldDO.getReceiveCount() == null ? 0 : oldDO.getReceiveCount();//原实收数量
                Long receiveCount = tAdvanceDeliveryDetail.getRegistrationCount() + oldReceiveCount;//现总共入库数量
                if (receiveCount > (oldDO.getPredictCount() - oldDO.getDetectionCount())) {
                    return AjaxResult.error(oldDO.getMaterialName() + "不可超出预计入库数量");
                }
                if (receiveCount.equals(oldDO.getPredictCount() - oldDO.getDetectionCount())) {//全部登记
                ...
                registration.setActualCount(0L);
    ...
    public AjaxResult checkTAdvanceDelivery(TAdvanceDeliveryDTO tAdvanceDelivery) {
        ...
        if (!CollectionUtils.isEmpty(deliveryDetailList)) {
            ...
            for (TAdvanceDeliveryDetail tAdvanceDeliveryDetail : deliveryDetailList) {
                ...
                //判断数量
                Long oldDetectionCount = oldDO.getDetectionCount() == null ? 0 : oldDO.getDetectionCount();
                //总不合格数量
                Long detectionCountSum = tAdvanceDeliveryDetail.getDetectionCount();
                if (detectionCountSum > oldDO.getPredictCount()) {
                    return AjaxResult.error(oldDO.getMaterialName() + "检测失败数量不可超出预计入库数量");
                }
                //本次不合格数量
                Long detectionCountNow = tAdvanceDeliveryDetail.getDetectionCount() - oldDetectionCount;
                if (detectionCountNow != 0) {
                    //生成拒收
    ...
    public AjaxResult registerCount(List<TAdvanceDeliveryApiDTO> advanceDeliveryList) {
        for (TAdvanceDeliveryApiDTO tAdvanceDeliveryApiDTO : advanceDeliveryList) {
            Long predictCount = tAdvanceDeliveryApiDTO.getCount();//入库数量
            ...
            if (!CollectionUtils.isEmpty(detailByMaterial)) {
                Long predictCountSum = detailByMaterial.stream().mapToLong(TAdvanceDeliveryDetailVO::getPredictCount).sum();
                Long receiveCountSum = detailByMaterial.stream().mapToLong(TAdvanceDeliveryDetailVO::getReceiveCount).sum();
                Long residueCountSum = predictCountSum - receiveCountSum;
                ...
                for (TAdvanceDeliveryDetailVO e : detailByMaterial) {
                    ...
                    Long oldpredictCount = e.getPredictCount();//原预计数量
                    Long oldReceiveCount = e.getReceiveCount();//原实收数量
                    Long residueCount = oldpredictCount - oldReceiveCount;//现单剩余数量
                    if (predictCount >= residueCount) {//全部登记
                        tAdvanceDeliveryDetail.setNextFlag(Constants.INOUT_STATUS_END);
                        tAdvanceDeliveryDetail.setReceiveCount(oldpredictCount);
                        tAdvanceDeliveryDetail.setRegistrationCount(oldpredictCount);
                        predictCount = predictCount - residueCount;
                    } else {//部分登记
                        tAdvanceDeliveryDetail.setNextFlag(Constants.INOUT_STATUS_PART);
...
//                        tAdvanceDeliveryDetail.setReceiveCount(oldReceiveCount + predictCount);
    ...
    public AjaxResult checkDeliveryMaterial(TAdvanceDelivery tAdvanceDelivery) {
        ...
        listByDeliveryIds.forEach(e -> {
            int count = materialDetailService.updateStatusByDeliveryId(e.getId());
            //更新入库单详情的检测数量
            TAdvanceDeliveryDetail updateDeliveyDetail = new TAdvanceDeliveryDetail();
            updateDeliveyDetail.setId(e.getId());
            updateDeliveyDetail.setDetectionCount(Long.valueOf(count));
    ...
    public void generateOtherMaterialDetail(Long deliveryId) {
        ...
        for (TAdvanceDeliveryDetail detail : deliveryDetailList) {
            Long diffCount = detail.getRegistrationCount() - detail.getPredictCount();
            if (diffCount > 0) {
    ...
    public String getDetailCountStatus(Long deliveryId, int type) {
        ...
        if (CollectionUtils.isNotEmpty(tAdvanceDeliveryDetails)) {
            if (type == 1) {
                ...
                List<TAdvanceDeliveryDetail> endCount = tAdvanceDeliveryDetails.stream().filter(e -> e.getPredictCount() <= e.getRegistrationCount()).collect(Collectors.toList());
                if (tAdvanceDeliveryDetails.size() == endCount.size()) {
    ...
    public AjaxResult bomAdd(TBomDetail tBomDetail) {
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            if (redusicCount > 0) {
                ...
                tAdvanceDeliveryDetail.setPredictCount(redusicCount);
