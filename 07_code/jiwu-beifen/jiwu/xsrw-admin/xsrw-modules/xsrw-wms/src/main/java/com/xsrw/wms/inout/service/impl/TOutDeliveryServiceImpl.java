    ...
    public AjaxResult insertTOutDelivery(TOutDeliveryVO tOutDeliveryVO)
    {
        ...
        for (TOutDeliveryDetail tOutDeliveryDetail : collect) {
            ...
            if (tOutDeliveryDetail.getPredictCount() != null){
                ...
                // 货架库存
                Integer materialTray = tStockMapper.getMaterialNum(tOutDeliveryDetail.getMaterialId(), "0");
                // 地堆库存
                Integer materialLocal = tStockMapper.getMaterialNum(tOutDeliveryDetail.getMaterialId(), "1");
                if (tOutDeliveryDetail.getPredictCount() > materialTray
                        && tOutDeliveryDetail.getPredictCount() > materialLocal){
            ...
            //判断小件领取数量是否超过包装单位最大数据--张雅倩
             if(tOutDeliveryDetail.getSmallPredictCount() != null){
                ...
                if(tOutDeliveryDetail.getSmallPredictCount() >= unitConfig.getCount()){
                    return AjaxResult.error("小件领取数已足整件领取");
                }
            }
            tOutDeliveryDetail.setPredictReceiveCount(tOutDeliveryDetail.getPredictCount());
            if(tOutDeliveryDetail.getPredictCount() == null){
                tOutDeliveryDetail.setPredictReceiveCount(1l);
            }
            if(tOutDeliveryDetail.getPredictCount() != null && tOutDeliveryDetail.getSmallPredictCount() != null){
                tOutDeliveryDetail.setPredictReceiveCount(tOutDeliveryDetail.getPredictReceiveCount()+1);
            }
            if(tOutDeliveryDetail.getPredictReceiveCount()>stockMain.getAvailableCount()){
            ...
            //更新该物料的在库可用数量
            stockMain.setAvailableCount(stockMain.getAvailableCount()-tOutDeliveryDetail.getPredictReceiveCount());
    ...
    public List<TMaterialSelectVO> getMaterialSelectList(TMaterialDTO tMaterial) {
        ...
        if (materialList.size() > 0){
            ...
            materialList.forEach(e -> {
                // 查询货架
                Integer materialTray = tStockMapper.getMaterialNum(e.getId(), "0");
                // 查询地堆
                Integer materiaLocal = tStockMapper.getMaterialNum(e.getId(), "1");
    ...
    public AjaxResult updateTOutDelivery(TOutDeliveryVO tOutDelivery)
    {
        ...
        for (TOutDeliveryDetail data:tOutDeliveryDetails) {
            ...
            if(stockMain!=null && data.getSmallPredictCount() == null){
                stockMain.setAvailableCount(stockMain.getAvailableCount() + data.getPredictCount());
                stockMainService.updateById(stockMain);
            }
        }
...
//        this.insertTOutDelivery(tOutDelivery);
        ...
        for (TOutDeliveryDetail tOutDeliveryDetail : collect) {
            ...
            //更新该物料的在库可用数量
            stockMain.setAvailableCount(stockMain.getAvailableCount()-tOutDeliveryDetail.getPredictReceiveCount());
            if(stockMain.getAvailableCount()<0){
    ...
    public AjaxResult deleteTOutDeliveryByIds(Long[] ids)
    {
        ...
        List<TOutDeliveryDetail> outDeliveryDetailList = tOutDeliveryDetailMapper.selectBatchIds(Arrays.asList(ids));
        if(outDeliveryDetailList.size() > 0){
            ...
            for (TOutDeliveryDetail data:outDeliveryDetailList) {
                ...
                TStockMain stockMain = stockMainService.getOne(new QueryWrapper<TStockMain>()
                        ...
                        .eq("del_flag", Constants.DEL_FLAG_NO));
                // 更新库存中物料的可用数量
                stockMain.setAvailableCount(stockMain.getAvailableCount() + data.getPredictReceiveCount());
                stockMainService.updateById(stockMain);
            }
        }
   ...
   /**
    ...
    public AjaxResult suitAdd(TOutDeliveryVO tOutDeliveryVO) throws ServiceException {
        ...
        for (TOutDeliveryDetail tOutDeliveryDetail : tOutDeliveryDetailList) {
            ...
            tOutDeliveryDetail.setPredictReceiveCount(tOutDeliveryDetail.getPredictCount());
            if(tOutDeliveryDetail.getPredictReceiveCount()*tOutDeliveryVO.getSuitNum() > stockMain.getAvailableCount()){
            ...
            //更新该物料的在库可用数量
            stockMain.setAvailableCount(stockMain.getAvailableCount()-tOutDeliveryDetail.getPredictReceiveCount()*tOutDeliveryVO.getSuitNum());
            stockMainService.updateById(stockMain);
        }
...
//                }
                ...
                tStockDetail.setCurrentCount(tStock.getCount()-tTaskOutDetailListVO.getPredictCount());
                tStockDetail.setBeforeCount(tStock.getCount());// 操作前数量
                tStockDetailMapper.insert(tStockDetail);

                // 扣减库存
                tStock.setCount(tStock.getCount()-tTaskOutDetailListVO.getPredictCount());
                ...
                stockMain.setLibraryCount(stockMain.getLibraryCount() - tTaskOutDetailListVO.getPredictCount());
                stockMainService.updateById(stockMain);
            }
        }
        return AjaxResult.success();
...
//                tStockMain.setAvailableCount(tStockMain.getAvailableCount()-t.getActualCount());
    ...
    public AjaxResult visualOutbound(TOutDeliveryVO tOutDeliveryVO)
    {
        ...
        for (TOutDeliveryDetail tOutDeliveryDetail : collect) {
            ...
            if(tOutDeliveryDetail.getPredictCount()==null && tOutDeliveryDetail.getSmallPredictCount() == null
                    && tOutDeliveryDetail.getPredictCount() == 0 && tOutDeliveryDetail.getSmallPredictCount() == 0
            ){
                return AjaxResult.error("请输入计划出库数量");
            }
            //出库数量是否大于在库可用数量
...
//            stockMain.setLibraryCount(stockMain.getLibraryCount()-tOutDeliveryDetail.getPredictReceiveCount());
    ...
    public AjaxResult emergencyOutbound(TOutDeliveryVO tOutDeliveryVO){
        ...
        if(tOutDeliveryVO.getPredictCount()==null && tOutDeliveryVO.getSmallPredictCount() == null
                && tOutDeliveryVO.getPredictCount() == 0 && tOutDeliveryVO.getSmallPredictCount() == 0
        ){
        ...
        if(tOutDeliveryVO.getSmallPredictCount() != null){
            ...
            if(tOutDeliveryVO.getSmallPredictCount() >= unitConfig.getCount()){
                return AjaxResult.error("小件领取数已足整件领取");
            }
        }
        Set<Long> stockIds=collect.stream().map(TOutDeliveryDetail::getStockId).collect(Collectors.toSet());
        //获取选中库存的数据中总库存数量
        Long sumCount=stockService.getStockByMaterialList(tOutDeliveryVO.getMaterialId()).stream()
                .filter(tStock -> stockIds.contains(tStock.getId()))
                .mapToLong(TStock::getAvailableCount).sum();
        Long predictCont=tOutDeliveryVO.getPredictCount();
        if(tOutDeliveryVO.getSmallPredictCount()!=null){
            predictCont=predictCont+tOutDeliveryVO.getSmallPredictCount();
        }
        ...
        queryWrapper.eq(TStockMain::getDelFlag,Constants.DEL_FLAG_NO);
        TStockMain stockMain = stockMainService.getOne(queryWrapper);
        if(predictCont>stockMain.getAvailableCount()){
            return AjaxResult.error("预计出库数量不可大于在库可用数量");
        }
        //更新该物料的在库可用数量
        stockMain.setAvailableCount(stockMain.getAvailableCount()-predictCont);
    ...
    public AjaxResult delOutAll(TOutDelivery tOutDelivery) {
        ...
        outDeliveryDetails.forEach(e ->{
            Long predictCount = e.getPredictCount();
            TStockMain tStockMain = stockMainService.getOne(new QueryWrapper<TStockMain>().eq("material_id", e.getMaterialId()));
            tStockMain.setAvailableCount(tStockMain.getAvailableCount()+predictCount);
            stockMainService.updateById(tStockMain);
        });
