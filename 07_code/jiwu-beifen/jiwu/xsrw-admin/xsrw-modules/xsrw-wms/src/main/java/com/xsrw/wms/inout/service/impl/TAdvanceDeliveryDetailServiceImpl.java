...
public class TAdvanceDeliveryDetailServiceImpl extends ServiceImpl<TAdvanceDeliveryDetailMapper, TAdvanceDeliveryDetail> implements ITAdvanceDeliveryDetailService {
    ...
    public synchronized AjaxResult putaway(TAdvanceDeliveryDetailDTO tAdvanceDeliveryDetailDTO) {
        ...
        registrationMap.forEach((key1, deliveryDetail) -> {
            ...
            Long residueCount = deliveryDetail.getDetectionCount() - deliveryDetail.getPutawayCount();//剩余数量
            if (actualCount > residueCount) {
                throw new ServiceException("物料总上架数量不可超出预估上架数量");
            }
            if (taskIns.size() == 1 && deliveryDetail.getDetectionCount().equals(actualCount)) {
    ...
    public AjaxResult floorStocking(TAdvanceDeliveryDetailDTO tAdvanceDeliveryDetailDTO) {
        ...
        if (oldDO.getDetectionCount() <= oldDO.getPutawayCount()) {
        ...
        if (Constants.YES.equals(tAdvanceDeliveryDetailDTO.getFloorStatus())) {
            ...
            floorCount = tMaterialDetailMapper.updateInfoByIdsOrRelId(tAdvanceDeliveryDetailDTO, trayId);
        } else {
        ...
        tAdvanceDeliveryDetail.setReceiveCount(oldDO.getReceiveCount() + floorCount);
        if (oldDO.getDetectionCount().equals(tAdvanceDeliveryDetail.getReceiveCount())) {
            tAdvanceDeliveryDetail.setPutawayCount(oldDO.getDetectionCount());
            //全部执行
            tAdvanceDeliveryDetail.setNextFlag(Constants.INOUT_NEXTFLAG_EXE_END);
        } else {
            tAdvanceDeliveryDetail.setPutawayCount(oldDO.getPutawayCount() + Long.valueOf(floorCount));
            //部分执行
        ...
        taskIn.setActualCount(Long.valueOf(floorCount));
        ...
        //更新库存
        oldDO.setDetectionCount(Long.valueOf(floorCount));
    ...
    public void inStockByMaterial(TAdvanceDeliveryDetailVO tAdvanceDeliveryDetailVO, Long trayId, Long locationId) {
        ...
        Long count = tAdvanceDeliveryDetailVO.getDetectionCount();
        //库存详情
    ...
    public AjaxResult putawayTaskNew(TAdvanceRegistrationApiDTO tAdvanceRegistrationApiDTO) {
        ...
        for (TAdvanceDeliveryApiDTO tAdvanceDeliveryApiDTO : materialList) {
            ...
            if (!CollectionUtils.isEmpty(deliveryDetailList)) {
                Long checkCountSum = deliveryDetailList.stream().mapToLong(TAdvanceDeliveryDetail::getDetectionCount).sum();
                Long actualCountSum = deliveryDetailList.stream().mapToLong(TAdvanceDeliveryDetail::getPutawayCount).sum();
                Long residueCountSum = checkCountSum - actualCountSum;
                ...
                for (TAdvanceDeliveryDetail tAdvanceDeliveryDetail : deliveryDetailList) {
                    ...
                    Long residueCount = tAdvanceDeliveryDetail.getDetectionCount() - tAdvanceDeliveryDetail.getPutawayCount();//现单剩余数量
                    Long actualCount = 0L;
                    ...
                    if (count >= residueCount) {
                        ...
                        actualCount = tAdvanceDeliveryDetail.getPutawayCount() + count;
...
//            }
    ...
    public AjaxResult afreshPutaway(TAdvanceDeliveryDetailDTO tAdvanceDeliveryDetailDTO) {
        ...
        registrationMap.forEach((key1, deliveryDetail) -> {
            List<TTaskIn> taskIns = registerGroupMap.get(key1);
            Long actualCount = taskIns.stream().mapToLong(TTaskIn::getActualCount).sum();//实际数量
            Long residueCount = deliveryDetail.getActualCount();//剩余数量
            if (!actualCount.equals(residueCount)) {
    ...
    public void inStockComplete(List<TTaskInVO> tTaskInList) {
        ...
        tTaskInList.forEach(e -> {
            ...
            if (existCount == null) {
                ...
                tStockDetail.setBeforeCount(existCount);// 操作前数量
                tStockDetail.setCurrentCount(existCount + e.getActualCount());// 操作后当前数量
            }
            tStockDetailMapper.insert(tStockDetail);
            //一个物料一个托盘一条
            TStock model = stockService.getOne(new LambdaQueryWrapper<TStock>()
            ...
            if (ObjectUtils.isNotNull(model)) {
                ...
                // 更新库存数据
                stockUpdate.setId(model.getId());
                stockUpdate.setCount(model.getCount() + e.getActualCount());
                stockUpdate.setAvailableCount(model.getAvailableCount() + e.getActualCount());

                // 更新库存总数据
                stockMainUpdate.setId(stockMain.getId());
                stockMainUpdate.setLibraryCount(stockMain.getLibraryCount() + e.getActualCount().intValue());
                stockMainUpdate.setAvailableCount(stockMain.getAvailableCount() + e.getActualCount().intValue());
