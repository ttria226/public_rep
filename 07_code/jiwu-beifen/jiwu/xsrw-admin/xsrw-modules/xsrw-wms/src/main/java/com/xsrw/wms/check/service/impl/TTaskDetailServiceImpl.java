...
public class TTaskDetailServiceImpl extends ServiceImpl<TTaskDetailMapper, TTaskDetail> implements ITTaskDetailService
{
    ...
    public List<TaskDetailVO> selectTTaskDetailList(TaskDetailDTO taskDetailDTO)
    {
        ...
        if (Constants.TASK_TYPE_CHECK.equals(taskDetailDTO.getTaskType())){
            list = tTaskDetailMapper.selectTaskDetailListCheck(taskDetailDTO);
        }else {
        ...
        if (!CollectionUtils.isEmpty(list)) {
            list.forEach(e -> {
                ...
                if (e.getRfidHead() != null){
                    e.setCheckDifferenceCount(e.getActualCount().longValue() - e.getRfidHeadCount().longValue());
                }
     ...
     */
    @Override
    public AjaxResult performCheck(Long id, Long checkNum) {
        //判断是否已执行过
        ...
        if (Constants.TASK_DETAIL_STATUS_APPROVED.equals(status)) {
            ...
            tasks.forEach(e -> {
                ...
                listMap.forEach((k,v) -> {
                    ...
                    // 实际数量
                    int sum = v.stream().mapToInt(f -> f.getActualCount().intValue()).sum();

                    TCheckHistory history = v.get(0);
                    // 查询原库存数据
                    TStock stock = itStockService.getById(history.getStockId());
                    // 盘库差值
                    Long differCount = stock.getCount() - sum;

                    TStock stockNew = new TStock();
                    stockNew.setId(stock.getId());
                    stockNew.setCount(v.stream().mapToLong(TCheckHistory::getActualCount).sum());
                    //解冻
                    stockNew.setIsFreeze(Constants.STOCK_IS_FREEZE_NO);
                    stockNew.setOriginType("");
                    stockNew.setAvailableCount(stock.getAvailableCount() - differCount);
                    stocks.add(stockNew);
                    ...
                    stockDetail.setCurrentCount(v.stream().mapToLong(TCheckHistory::getActualCount).sum());
                    ...
                    stockMain.setLibraryCount((long) (stockMainOrg.getLibraryCount().intValue() - differCount.intValue()));
                    stockMain.setAvailableCount((long) stockMainOrg.getAvailableCount().intValue()-differCount.intValue());
                    tStockMainMapper.updateCountByParam(stockMain);

                    // 处理RFID
                    v.forEach(f -> {
                        Integer count = f.getRfidHeadCount();
                        Long actualCount = f.getActualCount();

                        // 扣除多余的RFID
                        if (count.intValue() > actualCount.intValue()){
                            int num = count.intValue() - actualCount.intValue();
                            List<TMaterialDetail> delList = tMaterialDetailMapper.selectList(
                            ...
                            List<Long> delId = delList.stream().map(TMaterialDetail::getId).collect(Collectors.toList());
                            ...
                            // 删除redis中rfid
                            List<TMaterialDetail> detailList = tMaterialDetailMapper.selectList(
                                    new QueryWrapper<TMaterialDetail>()
                                            // 已入库
                                            .eq("status", "1")
                                            .eq("rfid_head", f.getRfidHead())
                            ...
                            TMaterialDetail detail = delList.get(0);
                            ...
                            tMaterialDetailVO.setCount(detailList.size());
                            tMaterialDetailVO.setRfids(detailList.stream().map(TMaterialDetail::getRfid).collect(Collectors.toList()));
                            redisService.setCacheObject("wms:materialDetail:" + tMaterialDetailVO.getRfid(), tMaterialDetailVO);
                        }

                        // 补充RIFD
                        if (actualCount.intValue() > count.intValue()){
                            ...
                            int num = actualCount.intValue() - count.intValue();

                            // 添加物料详情
                            for (int i = 0; i < num; i++) {
                                TMaterialDetail materialDetail = detailList.get(0);
                                Long newRfid = Long.valueOf(materialDetail.getRfid())+(i+1);
                                materialDetail.setRfid(newRfid.toString());
                                materialDetail.setUpdateTime(null);
                                materialDetail.setUpdateBy(null);
                                materialDetail.setCreateBy(null);
                                materialDetail.setCreateTime(null);
                            ...
                            // 添加redis中RFID
                            List<TMaterialDetail> redisDetail = tMaterialDetailMapper.selectList(
                                    new QueryWrapper<TMaterialDetail>()
                                            // 已入库
                                            .eq("status", "1")
                                            .eq("rfid_head", f.getRfidHead())
                            ...
                            TMaterialDetailRedisVO tMaterialDetailVO = new TMaterialDetailRedisVO();
                            tMaterialDetailVO.setBatchCode(redisDetail.get(0).getBatchCode());
                            tMaterialDetailVO.setMaterialId(redisDetail.get(0).getMaterialId());
                            tMaterialDetailVO.setMaterialName(redisDetail.get(0).getMaterialName());
                            tMaterialDetailVO.setRfid(f.getRfidHead());
                            tMaterialDetailVO.setCount(redisDetail.size());
                            tMaterialDetailVO.setRfids(redisDetail.stream().map(TMaterialDetail::getRfid).collect(Collectors.toList()));
                            redisService.setCacheObject("wms:materialDetail:" + tMaterialDetailVO.getRfid(), tMaterialDetailVO);
                        }
    ...
    public AjaxResult approveAreaCheck(Long taskDetailId, List<Long> ids, String status) {
        ...
        if (Constants.CHECK_AREA_CONFIRM.equals(status)){
            ...
            historyList.forEach(history -> {
                ...
                if (stock != null){
                    // 盘库差值
                    Long differCount = stock.getCount()-history.getActualCount();
                    stock.setCount(history.getActualCount().longValue());
                    stock.setAvailableCount(stock.getAvailableCount()-differCount);
...
//                    stock.setOriginType("");
                    ...
                    stockDetail.setMaterialId(stock.getMaterialId());
                    stockDetail.setBeforeCount(stock.getCount());
                    stockDetail.setCurrentCount(stock.getAvailableCount()-differCount);
                    stockDetail.setType(Constants.TASK_TYPE_CHECK);
                    stockDetails.add(stockDetail);
...
//                        newStock.setProducedDate(getProduced(history.getBatchNumber(),material.getId()));
                        ...
                        if (stockMainOrg == null){
                            ...
                            stockMain.setLibraryCount((long) history.getActualCount().intValue());
                            stockMain.setAvailableCount((long) history.getActualCount().intValue());
                            tStockMainMapper.insert(stockMain);
                        }else {
                            TStockMain stockMain = new TStockMain();
                            stockMain.setMaterialId(material.getId());
                            stockMain.setDeptId(taskDetail.getDeptId());
                            stockMain.setLibraryCount((long) stockMainOrg.getLibraryCount().intValue()+history.getActualCount().intValue());
                            stockMain.setAvailableCount((long) stockMainOrg.getAvailableCount().intValue()+history.getActualCount().intValue());
                            tStockMainMapper.updateCountByParam(stockMain);
                        }
