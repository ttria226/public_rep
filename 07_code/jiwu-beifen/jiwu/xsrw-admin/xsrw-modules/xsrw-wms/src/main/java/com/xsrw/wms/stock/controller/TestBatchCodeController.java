...
//        List<ErpWmsBatchCode> wmsList = tStockMapper.selectWmsList("1");
        ...
        erp.setErpCode("test2401080003");
        erp.setWmsCode("2401080003");
        erp.setMaterialId(28228L);
        wmsList.add(erp);
        //type1:1对1
...
//        wmsList.add(erp);
        ...
        for (ErpWmsBatchCode erpWmsBatchCode : wmsList) {
            ...
            trayBatchCodeMap.forEach((key, value) -> {
                ...
                long sumCount = value.stream().mapToLong(TStock::getCount).sum();
                long avaCount = value.stream().mapToLong(TStock::getAvailableCount).sum();
