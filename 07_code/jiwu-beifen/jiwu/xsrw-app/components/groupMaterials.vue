					...
					_this.$http.postAction('/wms/inout/detail/alllist', {
						...
						if(_this.typeName=='组盘' && _this.orderId){
							...
							_this.materialList.map((item) => {
								let info = materialList.find((materialInfo) => { return  materialInfo.materialId == item.materialId && materialInfo.batchCode == item.batchCode})
								if(info){
