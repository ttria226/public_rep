		...
		<view :class="{ 'tableBox': true, 'noTop': !isScan }">
			<u-table border-color="#595959">
				...
				<u-tr v-for="(item,index) in materialList">
					...
					<u-td width="20%">
						<u-input class="input-style" v-if="typeName=='拣货'" v-model="item.rfidsCount" type="number" :border="true"   />
						<u-input class="input-style" v-else v-model="item.count" type="number" :border="true"  />
					</u-td>
					...
					_this.$http.postAction('/wms/inout/detail/alllist', {
						...
						if(_this.typeName=='收货' || _this.typeName=='拣货'){
							...
							_this.materialList.map((item) => {
								let info = materialList.find((materialInfo) => { return materialInfo.batchCode == item.batchCode && materialInfo.materialId == item.materialId})
								if(info){
									if(_this.typeName=='拣货'){
