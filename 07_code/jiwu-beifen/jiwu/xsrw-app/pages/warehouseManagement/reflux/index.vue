	...
	<view>
		...
		<view class="content">
			...
			<view class="content_cart" v-for="(item,index) in dataList" :key="index">
				...
				<button class="button" type="primary" v-if="item.refluxStatus == 0" @click="goDetail(item.id)">回流</button>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getApi(type) {
				this.$http.getAction("/wms/outApi/outDeliveryList", this.queryParams).then(res => {
