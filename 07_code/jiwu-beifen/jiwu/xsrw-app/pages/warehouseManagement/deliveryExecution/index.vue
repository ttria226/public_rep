<template>
	<view>
		...
		<view class="content" style="height: 100%;">
			<view :class="{ 'choose': page == '2', 'content_cart': true }" v-for="(item,index) in dataList" :key="index">
				...
				<button class="button" type="primary" v-if="page == '1'" @click="goexecuteDetail(item,'2')">地推拣货</button>
				<button class="button" v-if="item.taskStatus != '3'" type="primary" @click="goexecuteDetail(item,'1')">{{btnText}}</button>
			</view>
	...
	export default {
		...
		methods: {
			...
			goexecuteDetail(item,type){
				if (this.page == '2') {
					uni.navigateTo({
						url: '../outInStore/outInStore?id=' + item.id + '&trayCode=' + item.trayCode
					});
				} else {
					console.log(item)
					uni.navigateTo({
						url: '/pages/warehouseManagement/deliveryExecution/execute?info=' + encodeURIComponent(JSON.stringify(item)) + '&page=' + this.page + "&type=" + type
					});
				}
			}
		}
