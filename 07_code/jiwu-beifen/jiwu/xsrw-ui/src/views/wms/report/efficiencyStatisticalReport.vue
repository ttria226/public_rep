<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="efficiencyStatisticalReportList">
      ...
      <el-table-column label="类型" align="center" prop="type" :show-overflow-tooltip="true" width="100" >
        <template slot-scope="scope">
          <span>{{ scope.row.type == '1' ? '入库' : (scope.row.type == '2' ? '出库' : '') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="数量" align="center" prop="count" :show-overflow-tooltip="true" width="120" />
      <el-table-column label="花费时间（h）" align="center" prop="timeConsume" :show-overflow-tooltip="true" min-width="100" />
      <el-table-column label="日期" align="center" prop="createTime" :show-overflow-tooltip="true" min-width="180" />
    </el-table>
...
<script>
import { listEfficiencyStatistics } from "@/api/wms/efficiencyStatistics";
import {wms} from '@/utils/agent'
export default {
  name: "efficiencyStatisticalReport",
  ...
  data(){
    return{
      ...
      total: 0,
      // 效率统计表格数据
      efficiencyStatisticalReportList: [],
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  watch: {
    $route(to, form) {
      if (to.name == "efficiencyStatisticalReport") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listEfficiencyStatistics(this.queryParams).then((response) => {
        this.efficiencyStatisticalReportList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/report/center/efficiencyStatistics/export", {...this.queryParams,}, `efficiencyStatisticsReport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>