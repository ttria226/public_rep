<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="workStatisticalReportList">
      ...
      <el-table-column label="工作时间（h）" align="center" prop="timeConsume" :show-overflow-tooltip="true" min-width="100" />
...
<script>
import { listWorkStatistics } from "@/api/wms/workStatistics";
import {wms} from '@/utils/agent'
export default {
  name: "workStatisticalReport",
  ...
  watch: {
    $route(to, form) {
      if (to.name == "workStatisticalReport") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      ...
      listWorkStatistics(this.queryParams).then((response) => {
        this.workStatisticalReportList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/report/center/workStatistics/export", {...this.queryParams,}, `workStatisticsReport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>