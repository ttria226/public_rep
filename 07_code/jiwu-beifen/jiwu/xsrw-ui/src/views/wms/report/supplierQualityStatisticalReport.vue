<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="supplierQualityStatisticalReportList">
      ...
      <el-table-column label="质检数量" align="center" prop="detectionCount" min-width="120px" />
      <el-table-column label="质检通过数量" align="center" prop="detectionPassCount" min-width="120px" />
      <el-table-column label="通过率" align="center" prop="passRate" min-width="120px" >
...
<script>
import { listSupplierQualityReport } from "@/api/wms/supplierReport";
import { getAction } from "@/api/manage"
import {wms} from '@/utils/agent'

export default {
  name: "supplierQualityStatisticalReport",
  ...
  data() {
    return {
      ...
      total: 0,
      // 供应商质量统计报表表格数据
      supplierQualityStatisticalReportList: [],
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSupplierQualityReport(this.queryParams).then(response => {
        response.rows.map((item) => {
          let num = Number(item.passRate.split("%")[0])
          item.passRateNum = num
        })
        this.supplierQualityStatisticalReportList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + '/report/center/qualityReport/export', {...this.queryParams}, `qualityReport_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
