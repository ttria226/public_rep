      ...
      <el-form ref="detailForm" :model="detailForm" label-width="100px" style="padding-right: 30px">
        ...
        <el-form-item label-width="0px" v-if="detailForm.deliveryDetailList && detailForm.deliveryDetailList.length > 0">
          <el-table v-loading="loading" :data="detailForm.deliveryDetailList">
            ...
            <el-table-column label="已入库数量" align="center" prop="detectionCount" width="120"></el-table-column>
            <!-- <el-table-column label="状态" align="center" prop="status" width="120">
              <template slot-scope="scope">
                <span style="color: red;" v-if="scope.row.registrationCount == 0">{{ scope.row.detectionCount }}个通过</span>
                <span style="color: green;" v-else-if="scope.row.status">全部通过</span>
              </template>
            </el-table-column> -->
            <el-table-column label="上架时间" align="center" prop="createTime" width="150"></el-table-column>
...
<script>
import { getPut } from "@/api/inoutDelivery/inDelivery";

export default {
  name: "warehouseOrderDetailCom",
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true;
      getPut(this.detailId).then(response => {
