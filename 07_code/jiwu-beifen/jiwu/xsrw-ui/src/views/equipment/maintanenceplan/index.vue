    ...
    <el-table v-loading="loading" :data="maintainPlanList">
      ...
      >
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.maintenance_plan_type"
            :value="scope.row.planType"
          />
        </template>
      </el-table-column>
      <el-table-column label="保养类型" align="center" prop="maintenanceType">
        <template slot-scope="scope">
          <dict-tag
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      ...
      >
        ...
        <el-form-item label="计划类型" prop="planType">
          ...
          >
            <el-option
              v-for="dict in dict.type.maintenance_plan_type"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            />
        ...
        <el-form-item label="保养间隔" v-show="form.planType == 1">
          <el-input v-model="form.intervalDay" placeholder="请输入天数" v-intNumber maxlength="6">
        ...
        <!-- <el-row :gutter="10" class="mb8" v-if="!isShow">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="addDetailed()"
              v-hasPermi="['wms:takeDelivery:addDetail']"
              >新增行</el-button
            >
          </el-col>
        </el-row>
...
export default {
  ...
  name: 'maintainPlan',
  dicts: ["maintenance_plan_status", "repair_type", "maintenance_type", "maintenance_plan_type"],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.$http.getAction("/wms/maintanenceplan/list", this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getMaintainPlan(id).then(response => {
        ...
        this.form.planType = this.form.planType.toString()
        this.open = true;
        this.title = "修改保养计划";
      });
    },
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            ...
            this.$http.postAction("/wms/maintanenceplan", this.form).then(response => {
    ...
    handleStatus(row, status) {
      ...
      let msg = ""
      if (status === '0') {
        msg = "温馨提示：作废操作后不可撤回，是否确定作废该保养计划？"
      } else {
        msg = '是否确认启用该条数据？'
      }
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + '/maintanenceplan/export', { ...this.queryParams }, `maintainPlan_${new Date().getTime()}.xlsx`)
    },
    /** 选择设备返回操作 */
    setEquipment(row) {
