...
<!--        <el-input v-model="queryParams.code" placeholder="请输入单据编号" clearable @keyup.enter.native="handleQuery" />-->
<!--      </el-form-item>-->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['loan:register:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['loan:register:export']">导出</el-button>
      </el-col>
...
<!--      <el-table-column label="单据编号" align="center" prop="code" />-->
      ...
      <el-table-column label="登记数量" align="center" prop="registerCount" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleDetailed(scope.row)" v-hasPermi="['loan:register:add:query']" >查看</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['loan:register:add:edit']" >编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :disabled="isShow">
        ...
        <el-form-item label="登记数量" prop="registerCount">
          <el-input v-model="form.registerCount" placeholder="请输入登记数量" v-intNumber maxlength="6" />
        </el-form-item>
      </el-form>
...
<script>
import { listRegister, getRegister, addRegister, updateRegister, delRegister } from "@/api/equipment/loan/register";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        registerCount: [{ required: true, message: '请输入登记数量', trigger: 'blur' }],
      },
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + '/loan/register/export', { ...this.queryParams }, `register${new Date().getTime()}.xlsx`)
    },
    /** 选择设备返回操作 */
    setEquipment(row) {
