<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['stock:detail:ledgerExport']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="stockLedgerList">
      ...
      <el-table-column label="规格型号" align="center" prop="specifications" :show-overflow-tooltip="true" width="280" />
      ...
      <el-table-column label="类型" align="center" prop="type" :show-overflow-tooltip="true" min-width="120">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.stock_detail_type" :value="scope.row.type" />
        </template>
      </el-table-column>
      <el-table-column label="数量" align="center" prop="currentCount" :show-overflow-tooltip="true" width="100" />
      <el-table-column label="日期" align="center" prop="createTime" width="180" :show-overflow-tooltip="true" min-width="150" />
    </el-table>
...
<script>
import { listStockLedger } from "@/api/stockLedger/stockLedger";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStockLedger(this.queryParams).then((response) => {
        this.stockLedgerList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/stock/detail/ledgerExport", {...this.queryParams,}, `ledgerExport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>
