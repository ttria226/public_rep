<template>
  <el-dialog title="出库任务详情" :visible.sync="open" width="60%" append-to-body>
          ...
          <el-table-column label="拣货数量" align="center" prop="receiveCount" width="150" >
            <template slot-scope="scope">
              <el-form-item label-width="0px" :prop="'taskWcsDetailVOList.' + scope.$index + '.receiveCount'" :rules="detailRules.receiveCount">
                <el-input v-model="scope.row.receiveCount" placeholder="请输入实际拣货数量" v-intNumber size="small" maxlength="6" @blur="val => handleInputChange(val,'receiveCount',scope.$index,scope.row)"></el-input>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="计量单位" align="center" prop="unitName" width="100"></el-table-column>
          <el-table-column label="拣货rfid" align="center" prop="rfidString" min-width="180" v-if="detailType == 3"></el-table-column>
...
<!--          </el-table-column>-->
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="220" fixed="right" v-if="detailType == 3">
            <template slot-scope="scope">
              <el-button size="mini" type="text" icon="el-icon-setting" v-if="scope.row.receiveCount && scope.row.receiveCount > 0" @click="handleChooseRfid(scope.row)">选择物料详细</el-button>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="submitDetailForm" v-if="detailType == 1 || detailType == 2 || detailType == 3" :loading="execLoading">执行</el-button>
...
import { getOutDeliveryTaskDetail, enforcementOutDelivery } from "@/api/inoutDelivery/outDelivery";
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true;
      getOutDeliveryTaskDetail(this.detailId,{ type: 2 }).then(response => {
    ...
    handleChooseRfid(item){
      this.$emit("showMaterialRfid",{ locationId: item.locationId, batchCode: item.batchCode, materialId:item.materialId})
    },
    //清空物料rfid信息
    ...
    submitDetailForm(){
      ...
      if(this.detailType == '3'){
        this.detailForm.taskWcsDetailVOList.map((item) => {
          ...
          } else if(item.receiveCount !== '' && item.receiveCount !== null && item.rfids.length != item.receiveCount){
