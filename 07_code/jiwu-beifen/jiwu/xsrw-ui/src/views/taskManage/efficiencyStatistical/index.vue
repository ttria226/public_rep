<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="efficiencyStatisticalList">
      ...
      <el-table-column label="花费时间（h）" align="center" prop="timeConsume" :show-overflow-tooltip="true" min-width="120" />
...
<script>
import { listEfficiencyStatistics } from "@/api/taskManager/efficiencyStatistics";
import {wms} from '@/utils/agent'
export default {
  name: "efficiencyStatistical",
  ...
  watch: {
    $route(to, form) {
      if (to.name == "efficiencyStatistical") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      ...
      listEfficiencyStatistics(this.queryParams).then((response) => {
        this.efficiencyStatisticalList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/taskManager/efficiencyStatistics/export", {...this.queryParams,}, `efficiencyStatisticsExport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>