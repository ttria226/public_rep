    ...
    public AjaxResult checkMaterial(List<TMaterialDetail> tMaterialDetail) {
        ...
        tMaterialDetail.forEach(e -> {
            ...
            if (tMaterialDetailVO != null) {
                ...
                tRejectionDetail.setRejectionCount(new BigDecimal(1));//拒收数量
            ...
            if (CollectionUtils.isNotEmpty(materialDetails)) {
                ...
                detailMap.forEach((key, value) -> {
                    ...
                    if (CollectionUtils.isNotEmpty(data)) {
                        ...
                        tMaterialDetailVO.setCount(new BigDecimal(rfids.size()));
...
//        }
    ...
    @Override
    public AjaxResult getRfidList(String rfid) {
        ...
        // 查询对应的库存id
        TStock tStock = tStockMapper.selectOne(new QueryWrapper<TStock>()
                .eq("material_id", materialDetail.getMaterialId())
                .eq("batch_code", materialDetail.getBatchCode())
                .eq(materialDetail.getTrayId() != null,"tray_id", materialDetail.getTrayId())
                .eq("del_flag", Constants.DEL_FLAG_NO));

        JSONObject jsonObject = (JSONObject) JSONObject.toJSON(materialDetail);
        jsonObject.put("stockId",tStock.getId());
    ...
    * */
    @Override
    public List<TMaterialDetailSVO> selectSMMaterialDetailList(TMaterialDetailSDTO tMaterialDetailS1) {
        return tMaterialDetailMapper.selectSMMaterialDetailList(tMaterialDetailS1);
    }
    /*
     * 物料入库扫描详情
     * */
    @Override
    public List<TMaterialDetailSZiVO> selectSMMaterialDetailZiList(TMaterialDetailSZiDTO tMaterialDetailSZiDTO) {
        return tMaterialDetailMapper.selectSMMaterialDetailZiList(tMaterialDetailSZiDTO);
    }
...
//    }
    ...
    public List<TMaterialDetailVO> selectCheckMaterialDetailList(@Param("trayType") String  trayType,@Param("entity") TMaterialDetailVO tMaterialDetailVO){
        return  tMaterialDetailMapper.selectCheckMaterialDetailList(trayType,tMaterialDetailVO);
    }
}
