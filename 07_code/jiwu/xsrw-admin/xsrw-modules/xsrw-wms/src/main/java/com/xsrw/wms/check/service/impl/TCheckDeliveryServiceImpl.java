...
public class TCheckDeliveryServiceImpl extends ServiceImpl<TCheckDeliveryMapper, TCheckDelivery> implements ITCheckDeliveryService
{
    ...
    public AjaxResult checkData(List<Map<String,Object>> map) {
        ...
        for (Map<String, Object> objectMap : map) {
            ...
            detail.setActualCount(new BigDecimal(objectMap.get("actualCount").toString()));
    ...
    public AjaxResult checkdeliverySubmit(CheckDeliveryDTO checkDeliveryDTO) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (history.getActualCount() == null || history.getActualCount().compareTo(BigDecimal.ZERO) == -1){
...
//        }
        ...
        result.forEach(e -> {
            List<Map<String, Object>> list = tMaterialDetailMapper.selectMaps(
                    new QueryWrapper<TMaterialDetail>()
                            .select(" sum(rfid_count) as rfidCount,rfid_head as rfidHead")
                            .eq("tray_id", e.get("trayId"))
                            .eq("batch_code", e.get("batchNumber"))
                            .eq("material_id",e.get("materialId"))
                            // 已入库
                            .eq("status","1")
                            .eq("del_flag", Constants.DEL_FLAG_NO)
                            .groupBy("rfid_head")
            ...
            list.forEach(i -> {
                ...
                map.put("rfidHeadCount",i.get("rfidCount"));
                mapList.add(map);
            });
        });
    ...
    @Override
    public List<CheckDeliveryVO> selectCheckMaterialDetailList(CheckDeliveryDTO checkDelivery){
        return  tCheckDeliveryMapper.selectCheckMaterialDetailList(checkDelivery);
    }
}
