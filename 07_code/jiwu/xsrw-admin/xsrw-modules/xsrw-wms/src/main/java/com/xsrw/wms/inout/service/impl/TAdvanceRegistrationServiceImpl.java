...
public class TAdvanceRegistrationServiceImpl extends ServiceImpl<TAdvanceRegistrationMapper, TAdvanceRegistration> implements ITAdvanceRegistrationService {
    ...
    public AjaxResult putaway(TAdvanceRegistrationDTO tAdvanceRegistration) {
        ...
        registrationMap.forEach((key1, value1) -> {
            ...
            BigDecimal residueCount = value1.getPredictCount().subtract(value1.getActualCount());//剩余数量
            ...
            updatyeDO.setActualCount(actualCount.add(value1.getActualCount()));
    ...
    public AjaxResult putawayTask(TAdvanceRegistrationApiDTO tAdvanceRegistrationApiDTO) {
        ...
        for (TAdvanceDeliveryApiDTO tAdvanceDeliveryApiDTO : materialList) {
            ...
            if (!CollectionUtils.isEmpty(registrationList)) {
                BigDecimal predictCountSum = registrationList.stream().map(TAdvanceRegistration::getPredictCount).reduce(BigDecimal.ZERO,BigDecimal::add);
                BigDecimal actualCountSum = registrationList.stream().map(TAdvanceRegistration::getActualCount).reduce(BigDecimal.ZERO,BigDecimal::add);
                BigDecimal residueCountSum = predictCountSum.subtract(actualCountSum);
                ...
                for (TAdvanceRegistration tAdvanceRegistration : registrationList) {
                    ...
                    BigDecimal residueCount = tAdvanceRegistration.getPredictCount().subtract(tAdvanceRegistration.getActualCount());//现单剩余数量
                    ...
                    if (count.compareTo(residueCount) != -1) {
                        ...
                        actualCount = tAdvanceRegistration.getActualCount().add(count);
    ...
    public void inStockByMaterial(TAdvanceRegistrationVO tAdvanceRegistration, Long trayId, Long locationId) {
        ...
        BigDecimal count = tAdvanceRegistration.getPredictCount();
        //库存详情
