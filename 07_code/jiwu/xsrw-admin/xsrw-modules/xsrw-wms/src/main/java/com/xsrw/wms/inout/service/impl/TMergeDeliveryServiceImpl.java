...
public class TMergeDeliveryServiceImpl extends ServiceImpl<TMergeDeliveryMapper, TMergeDelivery> implements ITMergeDeliveryService
{
    ...
    public AjaxResult createMergeDelivery(Long[] ids) {
        ...
        if (outDeliveryDetails.size() > 0){
            // 合并物料数量
            Map<Long, List<TOutDeliveryDetail>> materialMapList = outDeliveryDetails.stream().collect(
                    Collectors.groupingBy(TOutDeliveryDetail::getMaterialId));

            Map<Long,BigDecimal> materialMap = new HashMap<>();
            materialMapList.forEach((k, v) -> {
                materialMap.put(k,v.stream().map(TOutDeliveryDetail::getPredictReceiveCount).reduce(BigDecimal.ZERO,BigDecimal::add));
            });
...
//            }
    ...
    public AjaxResult addTaskPile(TTaskOutVO tTaskOutVO) {
        ...
        //已拣货数量
        BigDecimal outboundCount = tTaskMergeMapper.selectList(Wrappers.lambdaQuery(TTaskMerge.class)
                .eq(TTaskMerge::getMergeDeliveryDetailId,tTaskOutVO.getOutDeliveryDetailId())
                .eq(TTaskMerge::getDelFlag,Constants.DEL_FLAG_NO)
        ).stream().map(taskMerge -> taskMerge.getActualCount()).reduce(BigDecimal.ZERO,BigDecimal::add);
...
//                tStockMain.setAvailableCount(tStockMain.getAvailableCount()-t.getActualCount());
    ...
    public AjaxResult executeOutTask(TTaskWcsOutVO tTaskWcsOutVO) {
        ...
        if(tTaskWcsOutVO.getReceiveCount() !=null) {
            ...
            if(tTaskWcsOutVO.getSmallReceiveCount() != null){
                stock.setAvailableCount(stock.getAvailableCount().add(BigDecimal.ONE));
                stock.setCount(stock.getCount().add(BigDecimal.ONE));
            }
            //更新载具、库位
            ...
            if(tTaskWcsOutVO.getSmallReceiveCount() != null){
                stockMain.setAvailableCount(stockMain.getAvailableCount().add(BigDecimal.ONE));
            }
            ...
            if(tTaskWcsOutVO.getSmallReceiveCount() != null){
                stockMain.setLibraryCount(stockMain.getLibraryCount().add(BigDecimal.ONE));
            }
        ...
        BigDecimal l1 = (tTaskWcsOutVO.getReceiveCount() != null) ? (tTaskWcsOutVO.getSmallReceiveCount() == null ? tTaskWcsOutVO.getReceiveCount() : (tTaskWcsOutVO.getReceiveCount().add(BigDecimal.ONE))) : (tTaskWcsOutVO.getSmallReceiveCount() != null ? BigDecimal.ONE : BigDecimal.ZERO);
        ...
        for (TTaskMerge tTaskMerge : tTaskMergeList) {
            ...
            if(tTaskMerge.getActualCount().compareTo(tTaskMerge.getPredictCount()) == -1){
                flag = false;
                break;
            }
        }
    ...
    public Map<String, Object> getOutDeliveryCount(String id) {
        ...
        BigDecimal OutboundCount=tTaskMergeMapper.selectList(Wrappers.lambdaQuery(TTaskMerge.class)
                ...
                .eq(TTaskMerge::getDelFlag,Constants.DEL_FLAG_NO)
        ).stream().map(taskMerge -> taskMerge.getActualCount()==null?BigDecimal.ZERO:taskMerge.getActualCount()).reduce(BigDecimal.ZERO,BigDecimal::add);
