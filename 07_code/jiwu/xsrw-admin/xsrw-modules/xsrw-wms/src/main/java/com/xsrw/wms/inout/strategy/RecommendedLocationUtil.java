...
//        }
    ...
    private Long recommendedLocationFirst(List<TLocation> locationList) {
        ...
        //去除暂存位，按层分组，如果某层大于1个，则推荐一个，否则不推荐
        Map<Long, List<TLocation>> map = locationList.stream()
                .filter(e -> !LocationConstants.WORKING_STORAGE_BIT.equals(e.getPalletNodeId()))
                .collect(Collectors.groupingBy(TLocation::getLocationPlies));
        for (Long locationPlies : map.keySet().stream().sorted().collect(Collectors.toList())) {
            if(map.get(locationPlies) != null && map.get(locationPlies).size() > 1) {
                return map.get(locationPlies).get(0).getId();
            }
        }
        return locationId;
    }
