...
public class TTaskInServiceImpl extends ServiceImpl<TTaskInMapper, TTaskIn> implements ITTaskInService {
    ...
    public int executeEndTask(List<TTaskInVO> tTaskInList) {
        ...
        taskInMap.forEach((detailId, taskInList) -> {
            ...
            //已上架数量
            BigDecimal putawayCountDelivery = taskInList.get(0).getPutawayCountDelivery();
            //检测成功数量
            BigDecimal detectionCountDelivery = taskInList.get(0).getDetectionCountDelivery();
            //已入库数量
            BigDecimal receiveCountDelivery = taskInList.get(0).getReceiveCountDelivery();
            Long deliveryId = taskInList.get(0).getOriginId();
            BigDecimal sumCount = taskInList.stream().map(TTaskIn::getActualCount).reduce(BigDecimal.ZERO,BigDecimal::add);
            BigDecimal redisCount = detectionCountDelivery.subtract(receiveCountDelivery);
            if (redisCount.compareTo(sumCount) == 0) {
                ...
                tAdvanceDeliveryDetail.setReceiveCount(sumCount.add(receiveCountDelivery));
            }
