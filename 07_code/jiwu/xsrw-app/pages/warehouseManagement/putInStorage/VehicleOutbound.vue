	...
	<view class="outbound">
		...
		<u-sticky>
			<view class="searchBox">
				...
				<u-search v-if="searchType == 0" placeholder="请输入载具编码" shape="square" v-model="queryParams.code" @search="search" @custom="search"></u-search>
		...
		<view class="tableBox" v-if="orderList && orderList.length > 0"><!--v-if="orderList && orderList.length > 0"-->
			<u-table border-color="#595959">
				...
				<u-tr v-for="(item,index) in orderList">
					...
					<u-td width="25%">{{transfrom(item.status)}}</u-td>
					<u-td width="25%"><view style="color: #009BF4;" @tap="showPage(item)">查看明细</view></u-td>
				</u-tr>
			</u-table>
		</view>
		<u-empty text="数据为空" mode="list" v-else></u-empty>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			showPage(item){
				console.log(item)
				let url = '/pages/warehouseManagement/putInStorage/vehicleListDetail?id='+item.id+'&code='+item.code+'&type='+item.trayCategory+'&page='+this.page+'&palletNum='+(item.palletNum ? item.palletNum : '')
				uni.navigateTo({
					url:url
				})
			},
			...
			getBaseList() {
				this.$http.getAction('/wms/api/base/tray/list', this.queryParams).then(res => {
