	...
	<view class="scanBox">
		...
		<view class="tableBox">
			<u-table border-color="#595959">
				<u-tr>
					<u-th width="15%">物料名称</u-th>
					<u-th width="15%">物料编号</u-th>
					<u-th width="25%">批次号</u-th>
					<u-th width="15%">预计</u-th>
					<u-th width="15%">已上架</u-th>
					<u-th width="15%">总计</u-th>
				</u-tr>
				<u-tr v-for="(item,index) in materialList">
					<u-td width="15%" class="u-tdsty">{{item.materialName}}</u-td>
					<u-td width="15%" class="u-tdsty">{{item.materialCode}}</u-td>
					<u-td width="25%" class="u-tdsty">{{item.batchCode}}</u-td>
					<u-td width="15%">
						<u-input class="input-style" v-model="item.predictCount" disabled type="number" :border="true" />
					</u-td>
					<u-td width="15%">
						<u-input class="input-style" v-model="item.putawayCount" disabled type="number" :border="true" />
					</u-td>
					<u-td width="15%">
				...
				<u-tr>
					...
					<u-th width="30%">RIFDS</u-th>
					<u-th width="30%">本次</u-th>
				</u-tr>
				<view v-for="(item,index) in materialList">
					<u-tr v-for="(ite,inx) in item.rfids">
						<u-td width="40%" class="u-tdsty">{{item.materialName}}</u-td>
						<u-td width="30%" class="u-tdsty">{{ite.rfids}}</u-td>
						<u-td width="30%">
							<u-input class="input-style" v-model="ite.num" disabled type="number" :border="true" />
						</u-td>
					</u-tr>
				</view>
						...
						if (_this.typeName == '组盘' && _this.orderId) {
							...
							_this.materialList.map((item) => {
								let info = materialList.find((materialInfo) => {
									console.log(55555,materialInfo)
								...
								if (info) {
									if (item.rfids) {
										...
										let rfidArr=[]
										item.rfids.map(ret=>{
											rfidArr.push(ret.rfids)
										})
										for (let i = 0; i < info.rfidHeads.length; i++) {
											if (rfidArr.includes(info.rfidHeads[i])) {
												isHas = true;
											}
										}
										if (info && !isHas) {
											let obj=[
												{
													rfids:info.rfidHeads[0],
													num:info.count
												}
											]
											item.rfids = [...item.rfids, ...obj]
											...
											item.actualCount?item.actualCount+= info.count:item.actualCount=info.count
