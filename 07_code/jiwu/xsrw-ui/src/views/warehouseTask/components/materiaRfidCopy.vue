  ...
  export default {
    ...
    methods: {
      ...
      checkSelectable(row, index) {
		  let typeValue
		  let rfid=[]
		  this.otherSelectList.map(res=>{
		  	if(res.rfidString&&res.rfidString.length>0){
		  		let rfidArr=res.rfidString.split(',')
		  		rfid=[...rfid,...rfidArr]
		  	}
		  })
		  const uniqueArray =[...new Set(rfid)]
        if(this.again){
          return uniqueArray.indexOf(row.rfidHead)==-1;
        }else{
          return (row.locationName === null || row.locationName === undefined)&&uniqueArray.indexOf(row.rfidHead)==-1;
        }
      },
      // 选中行
      ...
      submitForm() {
        ...
        this.selection.map((item) => {
          ...
          if (this.selection && this.selection.length > 0) {
            ...
            console.log(11111);
            this.$emit('setMaterialRfid', { rfids, id: this.id ,trayId:this.trayId,again:this.again,materialId:this.materialId,rfidCount:rfidCount })
          } else {
            this.open = false
          }
