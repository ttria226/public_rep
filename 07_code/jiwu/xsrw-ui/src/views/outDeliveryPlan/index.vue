...
<!--      <el-col :span="1.5">-->
<!--        <el-button type="primary" plain icon="el-icon-plus" size="mini" :disabled="multiple" @click="handleOutTask" v-hasPermi="['inout:deliveryOut:toOutTask']">生成出库任务</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" style="padding-right: 30px">
        ...
        <el-form-item prop="tOutDeliveryDetailList" label-width="30px">
          <el-table v-loading="loading" :data="form.tOutDeliveryDetailList">
            ...
            <el-table-column label="剩余库存" align="center" prop="stock" min-width="100"></el-table-column>
            <el-table-column label="预计出库数量" align="center" prop="predictCount" width="180">
              <template slot-scope="scope">
                <el-form-item label-width="0px" :prop="'tOutDeliveryDetailList.' + scope.$index + '.predictCount'">
                  <el-input-number :precision="3" :step="0.001" :max="999999" v-model="scope.row.predictCount" placeholder="请输入预计出库数量"  size="large" @blur="val => handleInputChange(val,'predictCount',scope.$index,scope.row)"></el-input-number>
                </el-form-item>
              </template>
            </el-table-column>
...
<script>
import { listOutDelivery, getOutDelivery, addOutDelivery, updateOutDelivery, delOutDelivery, getOutDeliveryTask, approveOutDelivery } from "@/api/inoutDelivery/outDelivery";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOutDelivery(this.queryParams).then((response) => {
    ...
    setMaterial(material) {
      ...
      let flag = tOutDeliveryDetailList.every((item,index) => {
        ...
        tOutDeliveryDetailList[this.detailIndex].stock = material.stock ? material.stock : 0;
    ...
    handleOutTask(){
      ...
      let ids = this.ids
      this.$modal.confirm("该操作将生成所选订单包含物料的出库任务,确定要生成吗？").then(function () {
        return getOutDeliveryTask({ ids: ids.toString() });
      }).then(() => {
        this.$modal.msgSuccess("生成出库任务成功");
        this.getList();
      }).catch(() => { });
    }
  },
