...
export default {
  ...
  methods: {
    ...
    setMaterial(material) {
      ...
      //     materialId: item.id,
      //     areaId: item.areaId,
      //     materialId: item.locationId
      //   }
      //   list.push(info)
      // })
      for (let i = 0; i < material.ids.id.length; i++) {
        let info = {
          materialId: material.ids.id[i],
          areaId: material.ids.areaId[i],
          reservoirId: material.ids.reservoirId[i],
          batchCode:material.ids.batchCode[i],
          locationId:material.ids.locationId[i]
        };
        list.push(info);
      }
      let params = {
