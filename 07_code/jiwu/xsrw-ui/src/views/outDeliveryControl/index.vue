	...
	<div class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
			...
			<el-form-item label="载具编号" prop="trayCode">
				<el-input v-model="queryParams.trayCode" placeholder="请输入载具编号" clearable
					@keyup.enter.native="handleQuery" />
			</el-form-item>
		...
		<el-row :gutter="10" class="mb8">
			<el-col :span="1.5">
				<el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleEmergencyOut"
					v-hasPermi="['inout:task:export']">新增应急出库</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
					v-hasPermi="['inout:task:export']">导出</el-button>
			</el-col>
		...
		<el-table v-loading="loading" :data="outDeliveryControlList">
			...
			<el-table-column label="目标库位" align="center" prop="locationName" :show-overflow-tooltip="true"
				width="120" />
			...
			<el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="250" fixed="right">
				<template slot-scope="scope">
					<el-button size="mini" type="text" icon="el-icon-document"
						@click="handleExecute(scope.row,4)">查看</el-button>
					...
					<el-button size="mini" type="text" icon="el-icon-postcard" v-if="scope.row.taskStatus != '3'"
						@click="handleRunDetail(scope.row)">运行记录</el-button>
					<el-button size="mini" type="text" icon="el-icon-setting"
						v-if="scope.row.taskStatus == '1' || scope.row.taskStatus == '4'"
						@click="handleExecute(scope.row,2)" v-hasPermi="['inout:task:checkDetail']">重新发送</el-button>
					<el-button size="mini" type="text" icon="el-icon-setting"
						v-if="scope.row.taskStatus != '3' && scope.row.taskStatus != '5' && scope.row.taskStatus != null"
						@click="handleExecute(scope.row,3)" v-hasPermi="['inout:task:executeOutTask']">强制完成</el-button>
					<el-button size="mini" type="text" icon="el-icon-delete"
						v-if="scope.row.taskStatus != '3' && scope.row.taskStatus != '5' && scope.row.taskStatus != null"
						@click="handleCancel(scope.row)">强制作废</el-button>
					<el-button size="mini" type="text" icon="el-icon-setting"
						v-if="scope.row.taskStatus == '2' && scope.row.taskStatus != null"
						@click="forcedBack(scope.row)">强制回库</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<!-- 详情/执行/重新执行/强制执行 -->
		<DetailCom ref="detailCom" @setControl="setControl" @showMaterialRfid="showMaterialRfid">
		</DetailCom>
		<!-- 选择物料rfid -->
		<material-rfid ref="outMaterialRfidCom" @setMaterialRfid="setMaterialRfid"></material-rfid>
		<!-- 应急出库 -->
		<emergency-out ref="emergencyOutCom" @showMaterial="showMaterial"
			@setEmergencyOut="setEmergencyOut"></emergency-out>
		<!-- 应急出库-选择物料 -->
		<MaterialCom ref="materialCom" @setMaterial="setMaterial"></MaterialCom>

		<!-- 查看任务运行记录 -->
		<RunTaskCom ref="runTaskCom" @setControl="setControl" @showRunTask="showRunTask" @closeTask="closeTask">
		</RunTaskCom>
		<!-- 查看wcs运行记录 -->
...
<script>
	import {
		listDeliveryExecute,
		updateDeliveryExecutePriority
	} from "@/api/inoutDelivery/inDelivery";
	import {
		cancelTaskOutDelivery,
		enforcementDeliveryOut,
		forcedBack
	...
	export default {
		...
		methods: {
			...
			/** 导出按钮操作 */
			handleExport() {
				this.download(wms + "/inout/task/export", {
					...this.queryParams,
			...
			handleExecute(row, type) {
				...
				if (type == 2) {
					...
					this.$refs.detailCom.taskNo = row.taskNo
				}
				// } else {
					...
					if (item.batchCode == materials.batchCode) {
						if (item.rfids) {
							...
							materials.rfids.map((materialRfidInfo) => {
								let info = rfidsList.find((itemRfidInfo) => {
									return itemRfidInfo == materialRfidInfo
								})
								if (!info) {
			...
			setMaterial(material) {
				...
				if (flag) {
					...
					this.$refs.emergencyOutCom.form.smallPredictCount = material.smallPredictCount ? material
						.smallPredictCount : "";
