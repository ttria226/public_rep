    ...
    <el-table v-loading="loading" :data="deliveryDetailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="250" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-edit" v-show="scope.row.status == '1'" @click="handleUpdate(scope.row)" v-hasPermi="['inout:delivery:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" v-show="scope.row.status == '1'" @click="handleDelete(scope.row)" v-hasPermi="['inout:delivery:remove']">删除</el-button>
<!--          <el-button size="mini" type="text" icon="el-icon-printer" v-if="scope.row.status == '2' || scope.row.status == '5'" @click="handlePrint(scope.row)" v-hasPermi="['inout:delivery:labelprint']">打印</el-button>-->
          <el-button size="mini" type="text" icon="el-icon-finished" v-show="scope.row.status == '2' || scope.row.status == '5'" @click="handleDetailed(scope.row, 3)" v-hasPermi="['inout:delivery:register']">登记</el-button>
          <el-button size="mini" type="text" icon="el-icon-document-checked" v-show="scope.row.status == '4'" @click="handleDetailed(scope.row, 2)" v-hasPermi="['inout:delivery:check']">检测</el-button>
          <el-button size="mini" type="text" icon="el-icon-document-checked" v-show="scope.row.status == '4'" @click="handleFinishTest(scope.row)" v-hasPermi="['inout:delivery:check']">快捷检验</el-button>
          <el-button size="mini" type="text" icon="el-icon-user" v-show="scope.row.status == '1'" @click="handleDetailed(scope.row, 1)" v-hasPermi="['inout:delivery:approve']">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
