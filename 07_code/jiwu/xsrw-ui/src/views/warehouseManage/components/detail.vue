  ...
  <div class="app-container">
    <el-dialog :title="detailTitle" :visible.sync="open" width="60%" append-to-body>
      <el-form ref="detailForm" :model="detailForm" label-width="100px" style="padding-right: 30px">
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="detail-form-item">
        ...
        <el-form-item label-width="0px" v-if="detailForm.deliveryDetailList && detailForm.deliveryDetailList.length > 0">
          <el-table v-loading="loading" :data="detailForm.deliveryDetailList">
            ...
            <el-table-column label="到期日期" align="center" prop="expireDate" min-width="150"></el-table-column>
            ...
            <el-table-column label="登记数量" align="center" prop="registrationCount" width="200" v-if="detailType == 3">
              <template slot-scope="scope">
                <el-form-item label-width="0px" :prop="'deliveryDetailList.' + scope.$index + '.registrationCount'" :rules="detailRules.registrationCount">
                  <el-input-number v-model="scope.row.registrationCount" placeholder="请输入登记数量"  size="large" :min="0.000" :max="99999999" :precision="3" :step="0.001" maxlength="6" @blur="val => handleInputChange(val,'registrationCount',scope.$index,scope.row)"></el-input-number>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column label="已登记数量" align="center" :prop="detailType == 3 ? 'registrationInCount' : 'registrationCount'" min-width="120" v-if="detailType == 2 || detailType == 3 || (detailType == 4 && detailForm.status >= 2 && detailForm.status != 9)"></el-table-column>
            <el-table-column label="检测不通过数量" align="center" prop="detectionCount" width="210" v-if="detailType == 2">
              <template slot-scope="scope">
                <el-form-item label-width="0px" :prop="'deliveryDetailList.' + scope.$index + '.detectionCount'" :rules="detailRules.detectionCount">
                  <el-input v-model="scope.row.detectionCount" placeholder="请输入检测不通过数量" v-intNumber size="small" maxlength="6" @blur="val => handleInputChange(val,'detectionCount',scope.$index,scope.row)"></el-input>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column label="未通过原因" align="center" prop="detectionFailType" width="210" v-if="detailType == 2">
              <template slot-scope="scope">
                <el-form-item label-width="0px">
                  <el-select style="width: 100%;" v-model="scope.row.detectionFailType" placeholder="请选择未通过原因" @change="val => handleSelectChange(val,'detectionFailType',scope.$index,scope.row)">
            ...
            <el-table-column label="备注" align="center" prop="detectionFailRemark" width="210" v-if="detailType == 2">
              <template slot-scope="scope">
                <el-form-item label-width="0px">
                  <el-input v-model="scope.row.detectionFailRemark" placeholder="请输入备注" size="small" maxlength="250" @blur="val => handleInputChange(val,'detectionFailRemark',scope.$index,scope.row)"></el-input>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column label="批次号" align="center" prop="batchCode" min-width="200"></el-table-column>
            <el-table-column label="状态" align="center" prop="nextFlag" width="210" v-if="detailType == 4 && detailForm.status >= 2 && detailForm.status != 9">
              <template slot-scope="scope">
                <span style="color: red;" v-if="scope.row.registrationCount == 0">未登记</span>
                <span style="color: green;" v-else-if="scope.row.predictCount == scope.row.registrationCount">已登记</span>
                <span style="color: green;" v-else>部分登记</span>
              </template>
            </el-table-column>
...
<!--            <el-table-column label="登记时间" align="center" prop="producedDate" width="210" v-if="detailType == 4 && detailForm.status >= 2 && detailForm.status != 9"></el-table-column>-->
        ...
        <el-form-item label="审核备注" v-if="detailType == 1">
          <el-input v-model="detailForm.auditRemark" type="textarea" :rows="5" placeholder="请输入内容" show-word-limit maxlength="250" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitDetailExamineForm('2')" v-if="detailType == 1">审核通过</el-button>
        <el-button type="danger" @click="submitDetailExamineForm('9')" v-if="detailType == 1">作废</el-button>
        <el-button type="primary" @click="submitDetailCheckForm" v-if="detailType == 2">确 定</el-button>
        <el-button type="primary" @click="submitDetailRegistrationForm" v-if="detailType == 3">登记入库</el-button>
<!--        <el-button type="primary" @click="submitDetailTemplateForm" v-if="detailType == 4 && detailForm.status >= 2 && detailForm.status != 9">标签打印</el-button>-->
  ...
  export default {
    ...
    data() {
      let validateRegistrationCount= (rule, value, callback) => {
        if (value === null || value === '') {
          callback(new Error("登记数量不能为空"));
          this.$message.error("登记数量不能为空")
        } else if (Number(value) < 0) {
          callback(new Error("登记数量应大于等于0"));
          this.$message.error("登记数量应大于等于0")
          ...
          callback(new Error("登记数量应小于预计入库数量"));
          this.$message.error("登记数量应小于预计入库数量")
        } else {
          callback();
        }
      };
      let validateDetectionCount= (rule, value, callback) => {
        if (value === null || value === '') {
          callback(new Error("检测不通过数量不能为空"));
          this.$message.error("检测不通过数量不能为空")
        } else if (Number(value) < 0) {
          callback(new Error("检测不通过数量应大于等于0"));
          this.$message.error("检测不通过数量应大于等于0")
          ...
          callback(new Error("检测不通过数量应小于预计入库数量"));
          this.$message.error("检测不通过数量应小于预计入库数量")
        } else {
          callback();
        }
      };
    ...
    methods: {
        ...
        getInDelivery(this.detailId).then(response => {
          ...
          if(this.detailType == '3'){
            this.detailForm.deliveryDetailList.map((item) => {
              ...
              item.registrationCount = item.predictCount - item.registrationInCount
            })
          }
          this.loading = false;
        });
      ...
      submitDetailRegistrationForm(){
        ...
        let errorMessage = []
        this.detailForm.deliveryDetailList.map((item) => {
          if(!this.isNoLimit && Number(item.registrationCount) > (Number(item.predictCount) - Number(item.receiveCount))){
            errorMessage.push(`物料【${(item.materialName)}】的登记数量应小于预计入库数量`)
          }
        })
        if(errorMessage.length > 0){
          this.$message.error(errorMessage.join("，"))
