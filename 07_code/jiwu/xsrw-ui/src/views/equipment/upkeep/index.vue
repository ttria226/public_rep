<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="保养类别" prop="maintenanceType">
        <el-select v-model="queryParams.maintenanceType" placeholder="请选择保养类别" clearable>
          <el-option v-for="dict in dict.type.maintenance_type" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="maintainRecordList">
      ...
      <el-table-column label="保养单号" align="center" prop="dayNo" min-width="200">
      ...
      <!-- <el-table-column label="规格型号" align="center" prop="dayNo" min-width="120" /> -->
      <el-table-column label="来源" align="center" prop="source">
        <template slot-scope="scope">
          <span v-if="scope.row.source == 1">保养计划</span>
      ...
      <el-table-column label="保养费用" align="center" prop="price" width="120" />
      <!-- <el-table-column label="消耗物料" align="center" prop="material" width="120" /> -->
      <el-table-column label="执行人" align="center" prop="executorName" width="120" />
      <!-- <el-table-column label="保养单位" align="center" prop="createBy" width="120" /> -->
      <el-table-column label="计划保养日期" align="center" prop="planDay" width="120">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.planDay, "{y}-{m}-{d}") }}</span>
        </template>
      </el-table-column>
      <el-table-column label="实际保养日期" align="center" prop="trueDay" width="120">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.trueDay, "{y}-{m}-{d}") }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column label="保养说明" align="center" prop="remark" width="120" /> -->
      ...
      <el-table-column label="操作" align="center" fixed="right" width="120">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleDetail(scope.row)" v-hasPermi="['wms:maintenanceDay:query']">保养详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form ref="form" :model="form" label-width="120px" :disabled="isShow">
        ...
        <el-form-item label="保养零配件名称">
          <el-input v-model="form.partName" placeholder="请输入保养零配件名称" />
        </el-form-item>
        <el-form-item label="保养内容">
          <el-input v-model="form.content" type="textarea" placeholder="请输入保养内容" />
        </el-form-item>
        <el-form-item label="计划保养时间">
          <el-input v-model="form.planDay" placeholder="请输入计划保养时间" />
        </el-form-item>
        <el-form-item label="保养类别">
          <el-select v-model="form.maintenanceType" placeholder="请选择保养类别" clearable>
            <el-option v-for="dict in dict.type.maintenance_type" :key="dict.value" :label="dict.label" :value="dict.value"/>
          </el-select>
          <!-- <el-input v-model="form.maintenanceType" placeholder="请输入保养类型" /> -->
        </el-form-item>
        <!-- <el-form-item label="是否外部保养">
        ...
        <el-form-item label="保养部门">
          <el-input v-model="form.planDay" placeholder="请输入保养部门" />
        </el-form-item>
        <el-form-item label="执行人">
          <el-input v-model="form.executorName" placeholder="请输入执行人" />
        </el-form-item> -->
        ...
        <el-form-item label="保养费用">
          <el-input v-model="form.price" placeholder="请输入保养费用">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.$http.getAction("/wms/maintenanceDay/baoyangedList", this.queryParams).then(response => {
