    ...
    <el-table v-loading="loading" :data="deliveryTurnoverReportList">
      ...
      <el-table-column label="类型" align="center" prop="type" min-width="120px" >
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.type == '2'">出库</span>
        </template>
      </el-table-column>
      <el-table-column label="数量" align="center" prop="count" min-width="120px" />
      <el-table-column label="时间" align="center" prop="createTime" min-width="150px" />
    </el-table>
...
export default {
  name: "inoutDeliveryTurnoverReport",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms+'/report/center/inOutStatement/export', {...this.queryParams}, `inOutStatement_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
