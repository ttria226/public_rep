<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="inoutDispatchList">
...
<script>
import { listInOutDispatch } from "@/api/dispatchCenter/inOutDispatch";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listInOutDispatch(this.queryParams).then((response) => {
        this.inoutDispatchList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/dispatchCenter/inOutDispatch/export", {...this.queryParams,}, `inOutDispatchExport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>