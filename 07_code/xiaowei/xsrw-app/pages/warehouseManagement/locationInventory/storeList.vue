		...
		methods: {
			...
			confirmSub(){
				let data = {
					...
					historyList: JSON.parse(JSON.stringify(this.$refs.paidan.materialList)),
				}
				data.historyList.map(item=> {
					item.actualCount = item.realyNum
				})
				console.log(data)
				// return
				this.$http.postAction('/wms/taskApi/checkList/submit', data).then(res => {
			...
			submit() {
				if (this.$refs.paidan.materialList && this.$refs.paidan.materialList.length > 0) {
					...
					for (let i = 0; i < materialList.length; i++) {
						if(materialList[i].predictCount!=materialList[i].realyNum){
							this.equation=true
							return
						}
					}
