<template>
	<view>
		<headTitle title="收货组盘"></headTitle>
		<VehicleList ref="vehicle" @closeOtherScan="closeOtherScanVehicle"></VehicleList>
		<view style="width: 100%;height: 10rpx;"></view>
		<scanMaterials ref="wuliao" typeName='收货' :orderId="id" @closeOtherScan="closeOtherScanMaterials"></scanMaterials>
		<view class="radioBox">
	...
	export default {
		...
		methods: {
			...
			closeOtherScanVehicle(){
				this.$refs.wuliao.close()
			},
			submit() {
				this.queryParams.trayCode = this.$refs.vehicle.scannerResult.scannerdata.trim()
				this.queryParams.materialList = this.$refs.wuliao.materialList || []
				if (this.queryParams.trayCode && this.queryParams.materialList && this.queryParams.materialList.length > 0 && this.queryParams.trayStatus) {
					...
					this.$refs.form.validate().then(v => {
						this.$http.postAction('/wms/api/inout/delivery/putawayinout/delivery/putawayComplete', data).then(res => {
