	...
	<view class="scanBox">
		...
		<view :class="{ 'tableBox': true, 'noTop': !isScan }">
			<u-table border-color="#595959">
				<u-tr>
					<u-th :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">物料名称</u-th>
					<u-th :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">批次号</u-th>
					<u-th :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">预计{{typeName}}数量</u-th>
					<u-th :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">实际{{typeName}}数量</u-th>
					<!-- <u-th v-if="typeName=='拣货' && isSmallChoose" width="20%">小件领取</u-th> -->
				</u-tr>
				<u-tr v-for="(item,index) in materialList">
					<u-td :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'" class="u-tdsty">{{item.materialName}}</u-td>
					<u-td :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'" class="u-tdsty">{{item.batchCode}}</u-td>
					<u-td :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">{{typeName=='拣货' ? item.receiveCount : item.expectedCount}}</u-td>
					<u-td :width="typeName=='拣货' && isSmallChoose ? '20%' : '25%'">
						<u-input class="input-style" v-if="typeName=='拣货'" v-model="item.rfidsCount" type="number" :border="true" disabled  />
						<u-input class="input-style" v-else v-model="item.count" type="number" :border="true" disabled  />
					</u-td>
	...
	export default {
		...
		methods: {
					...
					_this.$http.postAction('/wms/inout/detail/alllist', {
						...
						if(_this.typeName=='收货' || _this.typeName=='拣货'){
							...
							_this.materialList.map((item) => {
								let info = materialList.find((materialInfo) => { return materialInfo.batchCode == item.batchCode })
								if(info){
									if(_this.typeName=='拣货'){
						...
						console.log(777777,_this.materialList)
						_this.$forceUpdate()
					})
				})
			},
			...
			async setLoggerEnable() {
				...
				if(this.typeName=='拣货'){
					let respone = await this.$http.getAction('/wms/outApi/scanTray', {trayCode: this.trayCode})
					console.log(333333,respone.data)
