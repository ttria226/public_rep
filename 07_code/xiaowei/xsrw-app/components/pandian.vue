	...
	<view class="scanBox">
		...
		<view class="tableBox">
			<u-table border-color="#595959">
				<u-tr>
					<u-th width="25%" v-if="typeName != '库位盘点'">库位</u-th>
					<u-th width="25%">物料名称</u-th>
					<u-th width="25%" v-if="typeName == '库位盘点'">批次号</u-th>
					<u-th width="25%">库存数量</u-th>
					<u-th width="25%">实盘数量</u-th>
				</u-tr>
				<u-tr v-for="(item,index) in materialList">
					<u-td width="25%" class="u-tdsty" v-if="typeName != '库位盘点'">{{item.locationName}}</u-td>
					<u-td width="25%" class="u-tdsty">{{item.materialName}}</u-td>
					<u-td width="25%" class="u-tdsty" v-if="typeName == '库位盘点'">{{item.batchNumber}}</u-td>
					<u-td width="25%">{{ typeName == '库位盘点' ? item.predictCount : item.stockNum}}</u-td>
					<u-td width="25%">
				...
				jpushModulePandian.scannerRegister({
					...
					_this.$http.postAction('/wms/inout/detail/alllist', {
						...
						_this.materialList.map(ret=>{
							res.data.map(rej=>{
								ret.batchNumber==rej.batchCode?ret.realyNum=rej.count:''
							})
						})
						_this.$forceUpdate()
