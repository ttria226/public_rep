<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="taskQueryList">
      ...
      <el-table-column label="执行人" align="center" prop="createBy" :show-overflow-tooltip="true" min-width="120" />
...
<script>
import { listTaskQuery } from "@/api/taskManager/taskQuery";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTaskQuery(this.queryParams).then((response) => {
        this.taskQueryList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/taskManager/taskQuery/export", {...this.queryParams,}, `taskQueryExport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>