      ...
      <el-form ref="detailForm" :model="detailForm" label-width="100px" style="padding-right: 30px">
        ...
        <el-form-item label-width="0px" v-if="detailForm.tOutDeliveryDetailList && detailForm.tOutDeliveryDetailList.length > 0">
          <el-table v-loading="loading" :data="detailForm.tOutDeliveryDetailList">
            ...
            <el-table-column label="计量单位" align="center" prop="materialUnit" min-width="100"></el-table-column>
            ...
            <el-table-column label="已出库数量" align="center" prop="receiveCount" width="120"></el-table-column>
<!--            <el-table-column label="小件出库数量" align="center" prop="smallReceiveCount" width="120"></el-table-column>-->
            <!-- <el-table-column label="状态" align="center" prop="status" width="120" v-if="detailType == 2">
              <template slot-scope="scope">
                <template v-for="item in dict.type.wms_out_recheck_status">
                  <span :style="{ 'color': item.value == '3' ? 'green' : 'red' }" v-if="item.value === scope.row.status" :key="item.value">{{ item.name }}</span>
                </template>
              </template>
            </el-table-column> -->
            <el-table-column label="出库时间" align="center" prop="createTime" width="150" v-if="detailType == 2"></el-table-column>
            <el-table-column label="财务凭证号" align="center" prop="financeVoucherNo" min-width="150" v-if="detailType == 2"></el-table-column>
            <!-- <el-table-column label="仓库" align="center" prop="createTime" width="150" v-if="detailType == 1"></el-table-column> -->
          </el-table>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="danger" @click="submitDetailExamineForm('2')" v-if="detailType == 1">作废</el-button>
...
<script>
import { getOutDeliveryCheck } from "@/api/inoutDelivery/outDelivery";

export default {
  name: "outDeliveryCheckDetailCom",
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true;
      getOutDeliveryCheck(this.detailId).then(response => {
