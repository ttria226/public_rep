          ...
          <el-table-column label="上架数量" align="center" prop="reActualCount" min-width="80">
            <template slot-scope="scope">
              <el-form-item label-width="0px" :prop="'taskInList.' + scope.$index + '.reActualCount'" :rules="detailRules.reActualCount">
                <el-input-number v-model="scope.row.reActualCount" :precision="3" :step="0.001" :max="999999" placeholder="请输入上架数量" size="small"  maxlength="6"  @blur="val => handleInputChange(val,'reActualCount',scope.$index,scope.row)"></el-input-number>
              </el-form-item>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      console.log(this.materialList,1111);
