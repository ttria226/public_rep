<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="supplierDeliveryStatisticalReportList">
      ...
      <el-table-column label="入库数量" align="center" prop="receiveCount" min-width="120px" />
...
<script>
import { listSupplierQeliveryReport } from "@/api/wms/supplierReport";
import { getAction } from "@/api/manage"
import {wms} from '@/utils/agent'

export default {
  name: "supplierDeliveryStatisticalReport",
  ...
  data() {
    return {
      ...
      total: 0,
      // 供应商交付统计报表表格数据
      supplierDeliveryStatisticalReportList: [],
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSupplierQeliveryReport(this.queryParams).then(response => {
        response.rows.map((item) => {
          let num = Number(item.passRate.split("%")[0])
          item.passRateNum = num
        })
        this.supplierDeliveryStatisticalReportList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + '/report/center/deliveryReport/export', {...this.queryParams}, `deliveryReport_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
