  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="日期" prop="createTime">
        <el-date-picker v-model="queryParams.createTime" type="date" placeholder="请选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="inventoryDailySettlementReportList">
      ...
      <el-table-column label="变动数量" align="center" prop="changeCount" :show-overflow-tooltip="true"  min-width="120" >
...
<script>
import { listStockDailySettlement } from "@/api/wms/stockDailySettlement";
import {wms} from '@/utils/agent'
export default {
  name: "inventoryDailySettlementReport",
  ...
  data(){
    return{
      ...
      total: 0,
      // 库存日结查询表格数据
      inventoryDailySettlementReportList: [],
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  watch: {
    $route(to, form) {
      if (to.name == "inventoryDailySettlementReport") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStockDailySettlement(this.queryParams).then((response) => {
        this.inventoryDailySettlementReportList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + "/stock/detail/stockDailySettlement/export", {...this.queryParams,}, `inventoryDailySettlementReport_${new Date().getTime()}.xlsx`);
    },
  }
}
</script>
