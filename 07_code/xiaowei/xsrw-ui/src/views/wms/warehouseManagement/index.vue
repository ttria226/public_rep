  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      <el-form-item label="仓库名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入仓库名称" clearable style="width: 240px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="仓库状态" clearable style="width: 240px">
          <el-option v-for="dict in dict.type.warehouse_status" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="warehouseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="仓库编号" prop="id" min-width="120" />
      <el-table-column label="仓库名称" prop="name" :show-overflow-tooltip="true" min-width="150" />
      <!-- <el-table-column label="权限字符" prop="roleKey" :show-overflow-tooltip="true" min-width="150" /> -->
      <!-- <el-table-column label="显示顺序" prop="roleSort" width="100" /> -->
      <el-table-column label="仓库状态" align="center" min-width="100">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.status" active-value="0" inactive-value="1" @change="handleStatusChange(scope.row)"></el-switch>
        </template>
      </el-table-column>
      <el-table-column label="联系人" prop="contactPerson" align="center" :show-overflow-tooltip="true" min-width="120" />
      <el-table-column label="联系电话" prop="phone" align="center" :show-overflow-tooltip="true" min-width="120" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item v-if="title == '修改仓库'">
          ...
          <el-input v-model="form.roleKey" disabled placeholder="请输入权限字符" />
        </el-form-item>
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="form.status">
            <el-radio v-for="dict in dict.type.warehouse_status" :key="dict.value" :label="dict.value">{{dict.label}}</el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
import { listWarehouse, getWarehouse, delWarehouse, addWarehouse, updateWarehouse, changeWarehouseStatus } from "@/api/wms/warehouse";
import { dataScope, deptTreeSelect } from "@/api/system/role";
...
export default {
  ...
  methods: {
    ...
    /** 根据角色ID查询菜单树结构 */
    getWarehouseMenuTreeselect(roleId) {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要' + text + '【' + row.name + '】仓库吗？').then(function() {
    ...
    handleAdd() {
      ...
      setTimeout(() => {
        ...
        this.handleCheckedTreeNodeAll(true,'menu')
      },500)
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      ...
      const roleMenu = this.getWarehouseMenuTreeselect(row.roleId);
      getWarehouse(roleId).then(response => {
    ...
    handleAuthUser: function(row) {
      ...
      this.$router.push("/base/warehouse/warehouseManagement-auth/user/" + roleId);
    },
    /** 提交按钮 */
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != undefined) {
            ...
            params.menuIds = this.getMenuAllCheckedKeys();
            updateWarehouse(params).then(response => {
            ...
            params.menuIds = this.getMenuAllCheckedKeys();
            addWarehouse(params).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download(wms + '/warehouse/export', { ...this.queryParams }, `role_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>