  ...
  <div class="app-container">
    <el-dialog title="新增盘点计划" :visible.sync="open" width="1200px" append-to-body>
      <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
        <el-form-item label="盘点计划名称" prop="planName">
      ...
      <el-table v-loading="loading" :data="materialList" @selection-change="handleSelectionChange">
        ...
        <!-- <el-table-column label="单位" align="center" prop="specifications" min-width="120" :show-overflow-tooltip="true" /> -->
        <!-- <el-table-column label="批次" align="center" prop="batchCode" min-width="150" :show-overflow-tooltip="true" /> -->
        <el-table-column label="库区" align="center" prop="reservoirName" min-width="120" :show-overflow-tooltip="true" />
        <el-table-column label="库存数量" align="center" prop="rfidCount" min-width="120" :show-overflow-tooltip="true" />
        <el-table-column label="最后盘点时间" align="center" prop="lastCheckTime" min-width="120" :show-overflow-tooltip="true" />
        <!-- <el-table-column label="入库时间" align="center" prop="createTime" min-width="150" :show-overflow-tooltip="true" /> -->
        <el-table-column label="库区id" align="center" prop="reservoirId" min-width="150" v-if="false" :show-overflow-tooltip="false" />
        <el-table-column label="库位id" align="center" prop="locationId" min-width="150" v-if="false" :show-overflow-tooltip="false" />
        <el-table-column label="区域ID" align="center" prop="areaId" min-width="150" v-if="false" :show-overflow-tooltip="false" />
      </el-table>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.areaIds = selection.map((item) => item.areaId);
      this.reservoirIds = selection.map((item) => item.reservoirId);
      this.batchCodes=selection.map((item) => item.batchCode);
      this.locationIds=selection.map((item) => item.locationId);
    },
    /** 提交按钮 */
    submitForm() {
      if(this.queryParams.planName=='' || this.queryParams.planName==null){
        this.$modal.msgError('请输入盘点计划名称');
        return false;
      }
      if (this.ids && this.ids.length > 0) {
        this.$emit('setMaterial', { ids:{id: this.ids,areaId:this.areaIds,reservoirId:this.reservoirIds,batchCode:this.batchCodes,locationId:this.locationIds},planName: this.queryParams.planName })
      } else {
