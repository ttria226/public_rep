	...
	<div class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
			...
			<el-form-item label="入库单号">
				<el-input v-model="queryParams.rukuCode" placeholder="请输入入库单号" clearable
		...
		<el-table v-loading="loading" :data="validityWarningList">
			<el-table-column label="入库单号" align="center" prop="rukuCode" :show-overflow-tooltip="true"
				min-width="230px" />
			...
			<el-table-column label="预计入库数量" align="center" prop="predictCount" :show-overflow-tooltip="true"
				min-width="100px" />
			<el-table-column label="已扫描数量" align="center" prop="saomiaoShuliang" :show-overflow-tooltip="true"
				min-width="130px" />
			...
			<el-table-column label="操作" align="center" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button size="mini" type="text" icon="el-icon-edit"
						@click="handleDetailed(scope.row)">详情</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
	...
	export default {
		name: "warehousingRegistrationScan",
		components: {  detail },
		dicts: [],
		data() {
			return {
		...
		methods: {
			// 打开配置明细
			handleDetailed(row) {
			  this.$refs.detail.rukuCode = row.rukuziCode;
			...
			getList() {
				...
				getAction(`${wms}/inout/detail/selectSMMaterialDetailList`, this.queryParams).then(response => {
