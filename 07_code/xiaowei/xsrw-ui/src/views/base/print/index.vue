<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5" style="padding-right: 70%">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['base:print:add']">新增</el-button>
      </el-col>
      <template>
    ...
    <el-table v-loading="loading" :data="printList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="预约单据号" align="center" prop="advanceDeliveryCode" width="150"
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" v-if="scope.row.printCount == 0"
            @click="handleUpdate(scope.row)" v-hasPermi="['base:print:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.printCount == 0"
            @click="handleDelete(scope.row)" v-hasPermi="['base:print:remove']">删除</el-button>
          <el-button size="mini" type="text" icon="el-icon-printer" @click="handlePrint(scope.row)">RFID打印</el-button>
          <el-button size="mini" icon="el-icon-printer" type="text" @click="createRfid(scope.row)"
            v-if="scope.row.rfidHeads == null">RFID生成</el-button>
          <el-button size="mini" icon="el-icon-printer" type="text" @click="handlePrint2(scope.row)">普通打印</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="单据编号" prop="advanceDeliveryCode">
          <el-input v-model="form.advanceDeliveryCode" :disabled="showDisable" @focus="materialComOpen"
            placeholder="请输入单据编号" />
        </el-form-item>
        <el-form-item label="物料名称" prop="materialName">
        ...
        <el-form-item label="换算关系" prop="unitConfig">
          <el-select style="width: 30%;" v-model="form.maxUnit" clearable filterable placeholder="请选择基本单位"
            @change="qualifyScoreInputmax">
            <el-option v-for="item in minUnitList" :key="item.id" :label="item.name" :value="item.id"
              :disabled="item.disabled"></el-option>
          </el-select>
          <el-input-number style="width: 30%;margin-left: 10px;" :precision="3" @input="qualifyScoreInput"
             @blur="hsblur" v-model="form.convertCount" placeholder="请输入换算值" min="0"
            maxlength="10" />
          <el-select style="width: 30%;margin-left: 10px;" v-model="form.minUnit" disabled filterable
            placeholder="请选择包装单位" @change="qualifyScoreInput">
        ...
        <el-form-item label="打印物料数量" prop="sumCount">
          <el-input v-model="form.sumCount" style="width: 40%;" placeholder="请输入本次打印数量" @input="qualifyScoreInput"
            oninput="value=value.replace(/[^\d]/g,'')" onblur="value=value>0?parseInt(value):''" maxlength="5">
            <i slot="suffix"
              style="margin-right: 10px;font-weight: bold;color: black;font-size: 16px;">{{ form.maxUnitName }}</i>
          </el-input>
        </el-form-item>
    ...
    <el-dialog title="单个打印" :visible.sync="batchPrintOpen" width="500px" append-to-body>
      <!--批量打印 -->
      <div style="width:100%;max-height: 480px;overflow-y: auto;padding-top: 30px;">
        <div id="batchPrintArea">
          <div class="code_box" style="margin-top: -38px;width: 200px;height: auto;">
            <div style="width:150px;height:170px;margin-top:-25px;display: block;text-align: center;"
              v-for="(item, index) in rfids" :key="item + index">
              <vue-qr style="margin-left:0px;" :size="150" :text="item"></vue-qr>
              <span
                style="font-weight:bold;font-size:16px;margin:0 auto;margin-top:-1px;margin-left:0px;text-align: center;">
                {{ mcode }}
              </span>
            </div>
          </div>
        ...
        <div style="width: 100%;text-align: center;z-index: 1;">
          <el-button size="mini" type="text" icon="el-icon-printer" v-print="print">打印</el-button>
        </div>
...
export default {
  ...
  methods: {
    ...
    hsblur() {
      ...
      if (value > 0) {
        this.form.convertCount = value > 0 ? parseFloat(value) : ''
      }
      let sycount = this.form.returnCount
      // console.log(sycount / value);
      if (sycount / value > 0) {
        this.form.sumCount = Math.ceil(sycount / value)
      }
    },
    /** 查询rfid打印记录列表 */
    getList() {
    ...
    submitForm() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (Math.ceil(sycount / value) > Math.ceil(this.form.returnCount)) {
    ...
    setMaterial(material) {
      ...
      if (material) {
        ...
        this.form.returnCount = (material.predictCount - material.returnCount).toFixed(3)
      } else {
