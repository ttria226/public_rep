...
import com.xsrw.wms.inout.domain.dto.*;
    ...
    public AjaxResult alllist(TMaterialDetail tMaterialDetail) {
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            list.forEach(e -> {
                ...
                tMaterialDetailVO.setRfid(detail.getRfidHead());
...
//    @RequiresPermissions("inout:detail:list")
    ...
    public TableDataInfo getRegistrationInfo(@PathVariable("deliveryId") Long deliveryId, TMaterialDetailSerachDTO tMaterialDetail) {
        startPage();
        if (tMaterialDetail.getDetectionFailStatus() != null){
            // 查询待上架rfid物料列表
            ...
            // 查询入库单详情
            List<TAdvanceDeliveryDetailVO> list = tAdvanceDeliveryDetailService.getRegistrationList(deliveryId);
            return getDataTable(list);
        }
    }
    ...
    public AjaxResult checkMaterial(@RequestBody List<TAdvanceDeliveryDetailDTO> dtoList) {
        if (CollectionUtils.isEmpty(dtoList)) {
            return AjaxResult.error("参数不全");
        }
        return tAdvanceDeliveryDetailService.checkMaterial(dtoList);
    }
...
//    }
     ...
     * @return
     */
    @GetMapping(value = "/getRfidInfo")
    public AjaxResult getRfidInfo(String rfid){
        return tMaterialDetailService.getRfidList(rfid);
    }
     ...
     * 物料入库扫描列表
     */
    @GetMapping(value = "/selectSMMaterialDetailList")
    public TableDataInfo selectSMMaterialDetailList(TMaterialDetailSDTO tMaterialDetailS1){
        startPage();
    List<TMaterialDetailSVO> list = tMaterialDetailService.selectSMMaterialDetailList(tMaterialDetailS1);
        return getDataTable(list);
    }
    ...
    * 物料入库扫描详情
    */
    @GetMapping(value = "/selectSMMaterialDetaiZilList")
    public TableDataInfo selectSMMaterialDetailZiList(TMaterialDetailSZiDTO tMaterialDetailSZiDTO){
        List<TMaterialDetailSZiVO > list = tMaterialDetailService.selectSMMaterialDetailZiList(tMaterialDetailSZiDTO);
        return getDataTable(list);
    }
     ...
     * 盘点计划创建选择物料列表
     */
    @GetMapping(value = "/selectMaterialDetailList")
    public TableDataInfo selectMaterialDetailList(StockListDTO stockListDTO){
        startPage();
        TMaterialDetailVO tMaterialDetailVO = new TMaterialDetailVO();
        tMaterialDetailVO.setBatchCode(stockListDTO.getBatchCode());
        tMaterialDetailVO.setMaterialCode(stockListDTO.getMaterialCode());
        tMaterialDetailVO.setMaterialName(stockListDTO.getMaterialName());
        tMaterialDetailVO.setBatchCode(stockListDTO.getBatchCode());
        tMaterialDetailVO.setLocationId(stockListDTO.getLocationId());
        List<TMaterialDetailVO > list = tMaterialDetailService.selectCheckMaterialDetailList(stockListDTO.getTrayType(),tMaterialDetailVO);
        return getDataTable(list);
    }
}
