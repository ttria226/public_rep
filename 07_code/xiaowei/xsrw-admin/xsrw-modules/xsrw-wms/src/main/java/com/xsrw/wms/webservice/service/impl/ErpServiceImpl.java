...
public class ErpServiceImpl implements ErpService {
    ...
    public List<InOutResponse> inOut(List<InOutRequest> inOutRequestList) {
        ...
        if (inOutRequestList != null && !inOutRequestList.isEmpty()) {
            ...
            sysDept.setWarehouseCode(requestInfo.getWERKS());//仓库标识
            sysDept.setFactoryCode(requestInfo.getLGNUM());//工厂标识
            R<SysDept> deptResult =  remoteDeptService.getDeptInfo(sysDept, SecurityConstants.INNER);
            if ("E".equals(type)) {
                //入库
                ...
                if(StringUtils.isNotNull(deptResult) && StringUtils.isNotNull(deptResult.getData())) {
                	tAdvanceDelivery.setDeptId(deptResult.getData().getDeptId());
                	tAdvanceDelivery.setDeptName(deptResult.getData().getDeptName());
                }
...
//					erpDto.setMATNR(erpDto.getMATNR().substring(10));
                    ...
                    if(StringUtils.isNotNull(deptResult) && StringUtils.isNotNull(deptResult.getData())) {
                    	detail.setDeptId(deptResult.getData().getDeptId());
                    	detail.setDeptName(deptResult.getData().getDeptName());
                    }
                ...
                if(StringUtils.isNotNull(deptResult) && StringUtils.isNotNull(deptResult.getData())) {
                	tOutDeliveryVO.setDeptId(deptResult.getData().getDeptId());
                	tOutDeliveryVO.setDeptName(deptResult.getData().getDeptName());
                }
                for (InOutRequest erpDto : inOutRequestList) {
                    InOutResponse inOutResponse = new InOutResponse();
