...
//                    }
     ...
     */
    @Override
    public Map<String, TMaterial> getMaterialByCodes(List<String> codes) {
        Map<String, TMaterial> resMap = new HashMap<>();
        if(!CollectionUtils.isEmpty(codes)){
            ...
            if(tMaterials != null && !tMaterials.isEmpty()){
                resMap = tMaterials.stream().collect(Collectors.toMap(TMaterial::getCode, Function.identity()));
            }
        }
        return resMap;
    }
    ...
    /**
     * 保存erp同步过来的物料单据中的没有的物料信息
     * @param erpMaterialDTO
     * @return
     */
    @Override
    @Transactional
    public Long saveInToMaterial(InOutRequest erpMaterialDTO) {
        TMaterial tMaterial = new TMaterial();
        if (erpMaterialDTO.getDMBTR() != null && erpMaterialDTO.getMENGE() != null) {
            BigDecimal price = erpMaterialDTO.getDMBTR().divide(erpMaterialDTO.getMENGE()).setScale(2, BigDecimal.ROUND_UP);
            tMaterial.setUnitPrice(price.doubleValue());
        }
        //---------------------------------查找单位
        String unitName = erpMaterialDTO.getMEINS();
        if (StringUtils.isNotEmpty(unitName)) {
            TUnit tUnit = tUnitMapper.selectOne(Wrappers.lambdaQuery(TUnit.class)
                    .eq(TUnit::getName, unitName)
                    .eq(TUnit::getDelFlag, "0")
            );
            if (tUnit == null) {
                tUnit = new TUnit();
                ...
                tUnitMapper.insert(tUnit);
                tMaterial.setUnitId(tUnit.getId());
            } else {
                tMaterial.setUnitId(tUnit.getId());
            }
        }
        //---------------------------------物料类别
        String categoryId = erpMaterialDTO.getMATKL();
        if (StringUtils.isNotEmpty(categoryId)) {
            TCategory tCategory = tCategoryMapper.selectOne(Wrappers.lambdaQuery(TCategory.class)
                    .eq(TCategory::getId, Long.valueOf(categoryId))
                    .eq(TCategory::getDelFlag, "0")
            );
            if (tCategory == null) {
                tCategory = new TCategory();
                tCategory.setId(Long.valueOf(categoryId));
                tCategory.setName(erpMaterialDTO.getWGBEZ());
                tCategory.setDelFlag("0");
                tCategoryMapper.insert(tCategory);
                tMaterial.setCategoryId(tCategory.getId());
            } else {
                tMaterial.setCategoryId(tCategory.getId());
            }
        }
        //---------------------------------供应商
        String contactsUnitId = erpMaterialDTO.getLIFNR();
        if (StringUtils.isNotEmpty(contactsUnitId)) {
            TContactsUnit tContactsUnit = tContactsUnitMapper.selectOne(Wrappers.lambdaQuery(TContactsUnit.class)
                    .eq(TContactsUnit::getId, Long.valueOf(contactsUnitId))
                    .eq(TContactsUnit::getDelFlag, "0")
            );
            if (tContactsUnit == null) {
                tContactsUnit = new TContactsUnit();
                tContactsUnit.setId(Long.valueOf(contactsUnitId));
                tContactsUnit.setName(erpMaterialDTO.getNAME1());
                tContactsUnitMapper.insert(tContactsUnit);
                tMaterial.setContactsUnitId(tContactsUnit.getId());
            } else {
                tMaterial.setContactsUnitId(tContactsUnit.getId());
            }
        }
        ...
        //---------------------------------名称规格
        tMaterial.setCode(erpMaterialDTO.getMATNR());
        tMaterial.setName(erpMaterialDTO.getMAKTX());
        tMaterial.setDescription(erpMaterialDTO.getMAKTG());
        int nameIndex = tMaterial.getDescription().indexOf("\\");
        if (nameIndex > 0) {
            tMaterial.setSpecifications(tMaterial.getDescription().substring(nameIndex + 1));
        }
        tMaterial.setBatchAttrId(tBatchAttr.getId());
        tMaterial.setBatchFlag("1");
        tMaterial.setInspectionMethod("1");
        tMaterial.setExpirationFlag("0");
        tMaterial.setMaterialWidth(0L);
