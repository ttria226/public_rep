...
public class TTaskOutServiceImpl extends ServiceImpl<TTaskOutMapper, TTaskOut> implements ITTaskOutService {
    ...
    @Override
    public List<TTrayVO> selectGroundPile(String materialId,String batchCode) {
        return tTaskOutMapper.selectGroundPile(materialId,batchCode);
    }
    ...
    public AjaxResult voluntarily(Long outDeliveryId, Long materialId) {
        ...
        tStock.setBatchCode(deliveryDetail.getBatchCode());
        tStock.setRemark(order);
    ...
    public Map<String, Object> getOutDeliveryCount(String id) {
        Map<String, Object> map = new HashMap<>();
        BigDecimal predictReceiveCount = BigDecimal.ZERO;
        ...
        ).stream().map(tTaskOut -> tTaskOut.getActualCount() == null ? BigDecimal.ZERO : tTaskOut.getActualCount()).reduce(BigDecimal.ZERO,BigDecimal::add);
...
//            }
            ...
            if (tStockMain != null) {
                ...
                tTaskOutDTO.setAvailableCount(BigDecimal.ZERO);
            }
//            //预计拣货数量
//            tTaskOutDTO.setReceiveCount(tTaskOut.getPredictCount());
    ...
    @Override
    public List<TMaterialDetail> getMaterialRfidList(Long locationId, String batchCode, String rfidHead,Long materialId,Long trayId) {
        QueryWrapper<TMaterialDetail> eq = new QueryWrapper<TMaterialDetail>()
                .eq("location_id", locationId)
                .eq("batch_code", batchCode)
                .eq("status", Constants.MATERIAL_DETAIL_STATUS_IN)
                .eq("material_id",materialId)
                .eq(trayId != null,"tray_id",trayId)
                .eq("del_flag", Constants.DEL_FLAG_NO);
...
//        }
        ...
        // 查询rfid
        List<TMaterialDetail> rfidList = materialDetailService.list(new QueryWrapper<TMaterialDetail>()
                .in("rfid", tTaskWcsOutVO.getRfidList())
                .eq("status", "1")
                .eq("del_flag", Constants.DEL_FLAG_NO)
        );
        if (rfidList.size() == 0){
            return AjaxResult.error("RFID标签不可用");
        }

        BigDecimal rfidNum = tTaskWcsOutVO.getReceiveCount();
        ...
        if (rfidNum.compareTo(taskOut.getPredictCount().subtract(taskOut.getActualCount() == null ? BigDecimal.ZERO:taskOut.getActualCount())) == 1){
...
//        }
        ...
        if (tTaskWcsOutVO.getReceiveCount() == null) {
            tTaskWcsOutVO.setReceiveCount(BigDecimal.ZERO);
        }
        BigDecimal stockOut = tTaskWcsOutVO.getSmallReceiveCount() == null ? tTaskWcsOutVO.getReceiveCount() : tTaskWcsOutVO.getReceiveCount().add(BigDecimal.ONE);
        if (stock.getCount().compareTo(stockOut) == -1 ) {
        ...
        if (tTaskWcsOutVO.getReceiveCount() != null) {
            ...
            if (deliveryDetail.getReceiveCount() != null){
                count = tTaskWcsOutVO.getReceiveCount().add(deliveryDetail.getReceiveCount());
            }else {
                count = tTaskWcsOutVO.getReceiveCount();
            }
            deliveryDetail.setReceiveCount(count);
            //减去载具库存
            stock.setAvailableCount(stock.getAvailableCount().subtract(tTaskWcsOutVO.getReceiveCount()));
            stock.setCount(stock.getCount().subtract(tTaskWcsOutVO.getReceiveCount()));
            if (stock.getCount().compareTo(BigDecimal.ZERO) == -1 || stock.getAvailableCount().compareTo(BigDecimal.ZERO) == -1) {
            ...
            //判断现有库存是否为0
            //判断剩余库存是否为0
            if (stock.getCount().compareTo(BigDecimal.ZERO) == 0 && stock.getAvailableCount().compareTo(BigDecimal.ZERO) == 0) {
                //该条库存标记为删除状态
            ...
            stockMain.setLibraryCount(stockMain.getLibraryCount().subtract(tTaskWcsOutVO.getReceiveCount()));
            if (stockMain.getLibraryCount().compareTo(BigDecimal.ZERO) == -1) {
                return AjaxResult.error("系统错误，库存不足！");
            }
...
//        long l1 = (tTaskWcsOutVO.getReceiveCount() != null) ? (tTaskWcsOutVO.getSmallReceiveCount() == null ? tTaskWcsOutVO.getReceiveCount() : (tTaskWcsOutVO.getReceiveCount() + 1)) : (tTaskWcsOutVO.getSmallReceiveCount() != null ? 1 : 0);
        ...
        taskOut.setActualCount((taskOut.getActualCount() == null?BigDecimal.ZERO : taskOut.getActualCount()).add(tTaskWcsOutVO.getReceiveCount()));
        if (taskOut.getPredictCount().compareTo(taskOut.getActualCount()) == 0){
            // 已完成
        ...
        for (TTaskOut tTaskOut : tTaskOutList) {
            ...
            if (tTaskOut.getActualCount().compareTo(tTaskOut.getPredictCount()) == -1) {
                flag = false;
                break;
            }
        }
        ...
        // 拣货数量
        BigDecimal subtract = tTaskWcsOutVO.getReceiveCount();

        // 处理物料RFID
        for (TMaterialDetail detail : rfidList) {
            if (subtract.compareTo(BigDecimal.ZERO) == 1){
                if (detail.getRfidCount().compareTo(subtract) == 1){
                    detail.setRfidCount(detail.getRfidCount().subtract(subtract));
                }else {
                    detail.setRfidCount(BigDecimal.ZERO);
                    // 已出库
                    detail.setStatus("2");
                }
                detail.setOutDeliveryDetailId(deliveryDetail.getId());
                subtract = subtract.subtract(detail.getRfidCount());
            }
        }

        materialDetailService.updateBatchById(rfidList);

        // 处理redis中rfid
        Map<String, List<TMaterialDetail>> listMap = rfidList.stream().collect(Collectors.groupingBy(TMaterialDetail::getRfidHead));
        listMap.forEach((key,value)->{
            ...
            if (collect.size() == 0){
                ...
                tMaterialDetailVO.setCount(detail.getRfidCount());
        ...
        //出库完成反馈
        //Sort 1=物料整托出库；2=需半料回库
        if (StringUtils.isNotEmpty(tTaskWcsOutVO.getTaskNo()) && tTaskWcsOutVO.getSort()!= null){
            this.outFeedback(tTaskWcsOutVO.getTaskNo() ,tTaskWcsOutVO.getSort());
        }

        //TODO WMS-ERP对接出入库
        if(Constants.WCS_EXECUTE_STATUS_END.equals(tTaskWcs.getTaskStatus())){
            //因为存在部分拣货，所以在单据全部拣货完成的时候再回传给erp
            wmsToErpUtils.inoutPut(Constants.WCS_TASK_TYPE_OUT, tTaskWcs.getId(), null, null);
        }
...
//        }
        ...
        if (tTaskWcsOutVO.getRfidListPda() == null && tTaskWcsOutVO.getRfidListPda().size() == 0) {
            return AjaxResult.error("RFID参数不可为空");
        }

        List<String> rfidCollect = tTaskWcsOutVO.getRfidListPda().stream().map(rfid -> rfid.get("rfid").toString()).collect(Collectors.toList());
        // 查询rfid
        List<TMaterialDetail> rfidList = materialDetailService.list(new QueryWrapper<TMaterialDetail>()
                .in("rfid", rfidCollect)
                .eq("status", "1")
        ...
        for (Map<String, Object> map : tTaskWcsOutVO.getRfidListPda()) {
            String rfid = map.get("rfid").toString();
            for (TMaterialDetail detail : rfidList) {
                if (rfid.equals(detail.getRfid())){
                    if (new BigDecimal(map.get("count").toString()).compareTo(detail.getRfidCount()) == 1){
                        throw new ServiceException("拣货数量不可超过RFID标签值");
                    }
                }
            }
        }
        ...
        // 拣货总数
        BigDecimal rfidNum = tTaskWcsOutVO.getReceiveCount();
        if (rfidNum.compareTo(BigDecimal.ZERO) != 1){
            throw new ServiceException("拣货数量不可为0");
        }
...
//        long l1 = (tTaskWcsOutVO.getReceiveCount() != null) ? (tTaskWcsOutVO.getSmallReceiveCount() == null ? tTaskWcsOutVO.getReceiveCount() : (tTaskWcsOutVO.getReceiveCount() + 1)) : (tTaskWcsOutVO.getSmallReceiveCount() != null ? 1 : 0);
        ...
        for (Map<String, Object> map : tTaskWcsOutVO.getRfidListPda()) {
            ...
            for (TMaterialDetail detail : rfidList) {
                if (rfid.equals(detail.getRfid())){
                    if (new BigDecimal(map.get("count").toString()).compareTo(detail.getRfidCount()) == 0){
                        detail.setRfidCount(BigDecimal.ZERO);
                        // 已出库
                        detail.setStatus("2");
                    }else {
                        detail.setRfidCount(detail.getRfidCount().subtract(new BigDecimal(map.get("count").toString())));
                    }
                    detail.setOutDeliveryDetailId(deliveryDetail.getId());
                }
            }
    ...
    public AjaxResult insertTTaskOut(TTaskOutVO tTaskOutVO) {
        ...
        if (sum.compareTo(receiveCount) != 0) {
...
//            }
            ...
            // 处理RFID
            String rfidHead = task.getRfidHead();
            TMaterialDetail materialDetail = tMaterialDetailMapper.selectOne(new QueryWrapper<TMaterialDetail>()
                    .eq("rfid_head", rfidHead).eq("status", Constants.MATERIAL_DETAIL_STATUS_IN));
            if (materialDetail == null){
                throw new ServiceException("rfid:"+rfidHead+"不可使用");
            }
            if (!materialDetail.getBatchCode().equals(tStock.getBatchCode()) ||
                !materialDetail.getMaterialId().equals(tStock.getMaterialId()) ||
                !materialDetail.getLocationId().equals(tStock.getLocationId())){
                throw new ServiceException(rfidHead+"不可使用");
            }
            if (materialDetail.getRfidCount().compareTo(task.getReceiveCount()) == -1){
                throw new ServiceException(rfidHead+"拣货数量不可超过标签对应数量");
            }

            BigDecimal subtract = materialDetail.getRfidCount().subtract(task.getReceiveCount());
            if (subtract.compareTo(BigDecimal.ZERO) == 0){
                materialDetail.setRfidCount(BigDecimal.ZERO);
                materialDetail.setStatus(Constants.MATERIAL_DETAIL_STATUS_OUT_NO);
            }else {
                materialDetail.setRfidCount(subtract);
            }
            tMaterialDetailMapper.updateById(materialDetail);
...
//                tStockMain.setAvailableCount(tStockMain.getAvailableCount()-t.getActualCount());
            ...
            stockDetail.setCurrentCount(stock.getCount().subtract(t.getActualCount()));
            ...
            // 更改库存明细
            stock.setCount(stock.getCount().subtract(t.getActualCount()));
            stock.setAvailableCount(stock.getAvailableCount().subtract(t.getActualCount()));
    ...
    public AjaxResult executeOutByTray(TTaskOutVO tTaskOut) {
        ...
        for (TTaskOutDetailListVO tTaskOutDetailListVO : tTaskOutDetailListVOS) {
            ...
            //剩余数量
            BigDecimal receiveCount = stockVo.getCount().subtract(tTaskOutDetailListVO.getReceiveCount());
            BigDecimal count = tTaskOutDetailListVO.getReceiveCount().multiply(new BigDecimal(-1));
            if (receiveCount.compareTo(BigDecimal.ZERO) == -1) {
            ...
            if (receiveCount.compareTo(BigDecimal.ZERO)==0) {
                stockService.deleteTStockById(stockVo.getId());
            } else {
                TStock stockUpdate = new TStock();
                stockUpdate.setId(stockVo.getId());
                stockUpdate.setCount(stockVo.getCount().add(count));
                stockUpdate.setAvailableCount(stockVo.getAvailableCount().add(count));
            ...
            if (stockMain != null) {
                ...
                //无数量删除
                if (stockMainUpdate.getLibraryCount().compareTo(BigDecimal.ZERO)==0 && stockMainUpdate.getAvailableCount().compareTo(new BigDecimal(0L))==0) {
    ...
    public void outFeedback(String taskNo , Integer sort){
        WcsOrderDTO dto  = new WcsOrderDTO();
        dto.setTaskNo(taskNo);
        dto.setStartStation(WcsReportUtil.stationOut);
        wcsReportUtil.sendWcsOutFeedback(dto , sort);
    }
}
