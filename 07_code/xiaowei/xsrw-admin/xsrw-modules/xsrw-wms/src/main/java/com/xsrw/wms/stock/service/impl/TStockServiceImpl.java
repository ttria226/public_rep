...
public class TStockServiceImpl extends ServiceImpl<TStockMapper, TStock> implements ITStockService {
    ...
    public AjaxResult getTrayNum(String trayCode) {
        ...
        BigDecimal sum = stockList.stream().map(e -> e.getCount()).reduce(BigDecimal.ZERO,BigDecimal::add);
...
//    }
    ...
    public void moveStock(TStockInDTO stockInDTO, Long stockId, Long targetLocationId, Long targetAreaId) {
        ...
        if (existCount == null) {
            ...
            tStockDetail.setCurrentCount(existCount.add(stockInDTO.getCount()));// 操作后当前数量
        }
...
//                .eq(TStock::getDelFlag, Constants.DEL_FLAG_NO));
        if(modelOrg != null){
            BigDecimal residueCount = modelOrg.getCount().subtract(stockInDTO.getCount());
            BigDecimal residueAvaCount = modelOrg.getAvailableCount().subtract(stockInDTO.getCount());
            if(residueCount.compareTo(BigDecimal.ZERO) == 0 && residueAvaCount.compareTo(BigDecimal.ZERO) == 0){
                tStockMapper.deleteTStockById(modelOrg.getId());
            } else{
                TStock stockUpdate = new TStock();
                // 更新库存数据
                stockUpdate.setId(modelOrg.getId());
                stockUpdate.setCount(modelOrg.getCount().add(stockInDTO.getCount()));
                stockUpdate.setAvailableCount(modelOrg.getAvailableCount().add(stockInDTO.getCount()));
        ...
        if(modelTarget != null){
            ...
            stockUpdate.setCount(modelTarget.getCount().add(stockInDTO.getCount()));
            stockUpdate.setAvailableCount(modelTarget.getAvailableCount().add(stockInDTO.getCount()));
            this.updateById(stockUpdate);
        }else{
...
//        }
    ...
    public void moveInfoStock(TStockInDTO stockInDTO) {
        //todo wxr 处理库存记录 物料+库位+批次号
        BigDecimal count = stockInDTO.getCount();
        TStockDetail tStockDetail = new TStockDetail();
        tStockDetail.setMaterialId(stockInDTO.getMaterialId());
        ...
        if(model != null){
            BigDecimal receiveCount = model.getCount().subtract(stockInDTO.getCount());
            BigDecimal aCount = model.getAvailableCount().subtract(stockInDTO.getCount());
            if(receiveCount.compareTo(BigDecimal.ZERO) == 0 && aCount.compareTo(BigDecimal.ZERO) == 0){
                //数据为0则删除此数据
                ...
                stockUpdate.setCount(model.getCount().add(stockInDTO.getCount()));
                stockUpdate.setAvailableCount(model.getAvailableCount().add(stockInDTO.getCount()));
        ...
        if(stockMain != null){
            BigDecimal receiveCount = stockMain.getLibraryCount().subtract(stockInDTO.getCount());
            BigDecimal aCount = stockMain.getAvailableCount().subtract(stockInDTO.getCount());
            if(receiveCount.compareTo(BigDecimal.ZERO) == 0 && aCount.compareTo(BigDecimal.ZERO) == 0){
                //数据为0则删除此数据
                ...
                TStockMain stockMainUpdate = new TStockMain();
                stockMainUpdate.setId(stockMain.getId());
                stockMainUpdate.setLibraryCount(stockMain.getLibraryCount().add(stockInDTO.getCount()));
                stockMainUpdate.setAvailableCount(stockMain.getAvailableCount().add(stockInDTO.getCount()));
                tStockMainMapper.updateById(stockMainUpdate);
            }
        }else{
            stockMain = new TStockMain();
     ...
     */
    @Override
    public TStockVO selectByLocationAndMaterial(Long materialId, String batchCode, String locationCode) {
        return tStockMapper.selectByLocationAndMaterial(materialId,batchCode,locationCode);
    }
