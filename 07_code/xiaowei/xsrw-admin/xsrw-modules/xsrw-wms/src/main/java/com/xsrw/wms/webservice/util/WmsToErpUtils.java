...
import com.xsrw.wms.webservice.domain.dto.erp2wms.XwRequest;
...
public class WmsToErpUtils {
    ...
    //用户编码--后续替换为正式
    private static final String ZZYHZH_CODE = "ZZE0038786";
    //仓库编码--后续替换为正式
    ...
    public void inoutPut(String type, Long wcsId, String zzdjbm, String zzdjhh) {
        ...
        try{
            tTaskInList.forEach(material -> {
                ...
                erpRequest.setZZYHZH(ZZYHZH_CODE);//用户编码
            ...
            codeMap.forEach((code, sendItem) -> {
                ...
                try {
                	...
                	request.setWcsId(wcsId);
                	request.setZzdjbm(zzdjbm);
                	request.setErpType(type);
                	request.setType("2");//1:机务库 2:小微库
                	String url = jiwuUrl+"/webservice/erp/callBack";
            		
            		String jsonString = HttpRequestUtil.doPost(url, JSON.toJSONString(request), "utf-8");
                    logger.info("=============httpClient 调用机务接口->jsonString=" + jsonString);
...
//        movementRequest.setLGORT(LGORT_WARE1);//库存地点-只有立体库才有移库任务
            movementRequest.setZZYHZH(ZZYHZH_CODE);//用户编码
